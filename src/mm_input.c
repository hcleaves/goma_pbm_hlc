...
static int look_forward_optional_until(
    FILE *ifp, const char *string, char *untilstring, char input[], const char ch_term);
/*
...
struct Variable_Initialization Var_init_mat[MAX_NUMBER_MATLS][MAX_VARIABLE_TYPES + MAX_CONC];
...
void read_input_file(struct Command_line_command **clc, int nclc) {
  ...
  if (!New_Parser_Flag) {
    if ((ifp = fopen(Input_File, "r")) != NULL) {
      ...
      if (look_for_optional(ifp, "Geometry Specifications", input, '=') == 1)
        GOMA_EH(GOMA_ERROR,
                "CGM not supported, there should be no Geometry Specifications section.");
    ...
    GOMA_EH(GOMA_ERROR,
            err_msg); /* The -newp was used but this executable wasn'g built for it, exit */
    exit(0);
#endif                  /* NEW_PARSER */
  }
  return;
...
int count_parameters(const char string[])
...
void look_for(FILE *ifp, const char *string, char input[], const char ch_term)
/*
...
int look_for_either(
    FILE *ifp, const char *string1, const char *string2, char input[], const char ch_term) {
  /*
          Scan the input file (reading in strings according to 'read_string(ifp,)'
  ...
  */
  ...
  if (strcmp(input, string1) == 0)
    return 1;
  if (strcmp(input, string2) == 0)
    return 2;
  return -1;
}
/**************************************************************************/

int count_list(FILE *ifp, /* file pointer to open goma input file */
               const char *string,
               char input[],
               const char ch_term,
  ...
  */
  ...
  if (status == -1)
    fprintf(stderr,
            "### EOF found while counting list of %s ###\n"
...
/**************************************************************************/
int look_for_optional(FILE *ifp, const char *string, char input[], const char ch_term)
/*
        Scan the input file (reading in strings according to 'read_string(ifp,)'
        specifications) until the character pattern in 'string' is matched.
...
int look_forward_optional(FILE *ifp, const char *string, char input[], const char ch_term)
/***********************************************************************
 *
...
void rd_file_specs(FILE *ifp, char *input) {
  ...
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "FEM file", ExoFile);
  ...
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Output EXODUS II file", ExoFileOut);
  ECHO(echo_string, echo_file);
  ...
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "GUESS file", Init_GuessFile);
  ECHO(echo_string, echo_file);
  ...
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "SOLN file", Soln_OutFile);
  ECHO(echo_string, echo_file);
  ...
  foundMappingFile =
      look_for_optional_string(ifp, "Domain Mapping File", DomainMappingFile, MAX_FNL);
  if (foundMappingFile == 1) {
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Domain Mapping File", DomainMappingFile);
    ECHO(echo_string, echo_file);
  }
  ...
  if (strcasecmp(input, "no") == 0) {
    ...
    GOMA_EH(GOMA_ERROR, "Bad specification for intermediate results");
  }
   ...
   *	Answer: "yes" or "no".
   */
  if (look_forward_optional(ifp, "Write initial solution", input, '=') == 1) {
    (void)read_string(ifp, input, '\n');
    strip(input);
...
void rd_genl_specs(FILE *ifp, char *input) {
  ...
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Output Level", Iout);
  ECHO(echo_string, echo_file);
  ...
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Number of Jacobian File Dumps",
           Number_Jac_Dump);
  ECHO(echo_string, echo_file);
#endif
  ...
  if (iread == 1) {
    ...
    if (nargs == 0) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Conformation Map", first_string);
      ECHO(echo_string, echo_file);
  ...
  while ((iread = look_forward_optional(ifp, "Initialize", input, '=')) == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %s %d %f", "Initialize", input,
             Var_init[Num_Var_Init].ktype, Var_init[Num_Var_Init].init_val);
    ECHO(echo_string, echo_file);
  ...
  while ((iread = look_forward_optional(ifp, "External Field", input, '=')) == 1 ||
    ...
    if (fscanf(ifp, "%s", input) != 1) {
      GOMA_EH(GOMA_ERROR,
              "error reading exodus II file Name or voxel file name for external field.");
    }
    strip(input);
     ...
     */
    if (!strcmp(input, "IMPORT_EV"))
      Num_Import_EV++;
    if (!strcmp(input, "IMPORT"))
      Num_Import_NV++;
   ...
   */
  Num_Export_XS = 0;
  while ((iread = look_forward_optional(ifp, "Export Field", input, '=')) == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Export Field", ex_id);
    ECHO(echo_string, echo_file);
  }
#ifndef LIBRARY_MODE
  if (Num_Export_XS > 0)
    GOMA_WH(-1, "Export Field only valid in LIBRARY_MODE!");
#endif
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %g", "Pressure Datum", upd->Pressure_Datum);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%le", &upd->Acoustic_Frequency) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Acoustic Frequency");
    }
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%le", &upd->Process_Temperature) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Process Temperature");
    }
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %g", "Process Temperature",
             upd->Process_Temperature);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%le", &upd->Light_Cosmu) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Light Incident Cos(Angle)");
    }
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %g", "Light Cosmu", upd->Light_Cosmu);
    ECHO(echo_string, echo_file);
  }
...
void rd_timeint_specs(FILE *ifp, char *input) {
  char echo_string[MAX_CHAR_ECHO_INPUT] = "\0";
  char *echo_file = Echo_Input_File;
  ...
  if (strcmp(input, "steady") == 0) {
    ...
    GOMA_EH(GOMA_ERROR, "unknown time integration option");
  }

  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Time integration", input);
  ECHO(echo_string, echo_file);
  ...
  if (pd_glob[0]->TimeIntegration != STEADY) {
    ...
    if (fscanf(ifp, "%le", &delta_t0) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading delta_t");
    }
    tran->Delta_t0 = delta_t0;

    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "delta_t", tran->Delta_t0);
    ECHO(echo_string, echo_file);

    look_for(ifp, "Maximum number of time steps", input, '=');
    if (fscanf(ifp, "%d", &max_time_steps) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading max time steps");
    }
    tran->MaxTimeSteps = max_time_steps;

    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Maximum number of time steps",
             tran->MaxTimeSteps);
    ECHO(echo_string, echo_file);

    look_for(ifp, "Maximum time", input, '=');
    if (fscanf(ifp, "%le", &time_max) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading maximum time");
    }
    tran->TimeMax = time_max;

    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Maximum time", tran->TimeMax);
    ECHO(echo_string, echo_file);

    look_for(ifp, "Minimum time step", input, '=');
    if (fscanf(ifp, "%le", &delta_t_min) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading minimum time step");
    }
    tran->Delta_t_min = delta_t_min;

    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Minimum time step", tran->Delta_t_min);
    ECHO(echo_string, echo_file);
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%le", &delta_t_max) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Maximum time step");
      }

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Maximum time step", delta_t_max);
      ECHO(echo_string, echo_file);
    }
    ...
    if (fscanf(ifp, "%le", &(tran->theta)) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading time step parameter");
    }

    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Time step parameter", tran->theta);
    ECHO(echo_string, echo_file);

    look_for(ifp, "Time step error", input, '=');
    if (fscanf(ifp, "%le", &eps) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Time step error, expected at least one float");
    }
    tran->eps = eps;

    /* initialize norm indicators */
    for (i = 0; i <= 10; i++)
      tran->use_var_norm[i] = 1;
    /*tran->use_var_norm[9] = 1;  for backwards Compatibility */
    read_line(ifp, input, FALSE);

    if (sscanf(input, "%d %d %d %d %d %d %d %d %d %d", &tran->use_var_norm[0],
               &tran->use_var_norm[1], &tran->use_var_norm[2], &tran->use_var_norm[3],
               &tran->use_var_norm[4], &tran->use_var_norm[5], &tran->use_var_norm[6],
               &tran->use_var_norm[7], &tran->use_var_norm[8], &tran->use_var_norm[9]) < 10) {
      GOMA_WH(GOMA_ERROR, "Defaulting on Time step error norm indicators\n");
    }

    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g %d %d %d %d %d %d %d %d %d %d",
             "Time step error", tran->eps, tran->use_var_norm[0], tran->use_var_norm[1],
             tran->use_var_norm[2], tran->use_var_norm[3], tran->use_var_norm[4],
             tran->use_var_norm[5], tran->use_var_norm[5], tran->use_var_norm[7],
    ...
    if (print_freq == 0) {
      if (fscanf(ifp, "%le", &print_delt) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Printing delta time");
      }
      tran->print_delt = print_delt;

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d %.4g", "Printing Frequency", print_freq,
               print_delt);
      ECHO(echo_string, echo_file);
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%le %le", &print_delt2_time, &print_delt2) != 2) {
          GOMA_EH(GOMA_ERROR, "error reading second frequency time");
        }

        snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g %.4g", "Second frequency time",
                 print_delt2_time, print_delt2);
        ECHO(echo_string, echo_file);
      }
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Printing Frequency", tran->print_freq);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%le", &tran->resolved_delta_t_min) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Minimum Resolved Time Step");
      }
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Minimum Resolved Time Step",
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%le", &tran->Courant_Limit) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Courant Number Limit");
      }
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Courant Number Limit",
               tran->Courant_Limit);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%s", input) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Restart Time Integration After Renormalization.");
      }
      strip(input);
      stringup(input);
      if (strcmp(input, "ON") == 0 || strcmp(input, "YES") == 0 || strcmp(input, "TRUE") == 0) {
        tran->Restart_Time_Integ_After_Renorm = TRUE;
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d",
               "Restart Time Integration After Renormalization",
               tran->Restart_Time_Integ_After_Renorm);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%le", &(tran->init_time)) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Initial Time");
      }
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Initial Time", tran->init_time);
      ECHO(echo_string, echo_file);
      if ((time_max - tran->init_time) <= 0.0) {
        GOMA_EH(
            -1,
            "Your maximum time is less than or equal to your initial time!"); // a condition which
                                                                              // may result in NAN's
                                                                              // in the esp_dot
                                                                              // struct
      }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%d", &tran->const_dt_after_failure) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Steps of constant delta_t after failure");
      }
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d",
               "Steps of constant delta_t after failure", tran->const_dt_after_failure);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%le", &tran->time_step_decelerator) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Time step decelerator");
      }

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Time step decelerator",
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%s", input) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Fill Weight Function string");
      }
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Fill Weight Function", input);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%s", input) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Fill Equation string");
      }
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Fill Equation", input);
      ECHO(echo_string, echo_file);
    }
  } /*   if(pd_glob[0]->TimeIntegration != STEADY) */
  ...
  if (iread == 1) {
    ...
    if (strcmp(input, "no") == 0) {
      ...
      GOMA_EH(GOMA_ERROR, "Bad specification for March to Steady State");
    }

    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %s", "March to Steady State", input);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %g", "Steady State Tolerance",
             tran->steady_state_tolerance);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    if (tran->MaxSteadyStateSteps < 2) {
      GOMA_EH(GOMA_ERROR, "error reading max steady state steps expected value > 1");
    }
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Maximum steady state steps",
             tran->MaxSteadyStateSteps);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %s", "ALE Adapt", input);
    ECHO(echo_string, echo_file);
  }
...
void rd_levelset_specs(FILE *ifp, char *input) {
  char echo_string[MAX_CHAR_ECHO_INPUT] = "\0";
  char *echo_file = Echo_Input_File;
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Level Set Interface Tracking", input);
    ECHO(echo_string, echo_file);
  }
  ...
  if (ls != NULL) {
    ...
    while ((iread = look_forward_optional(ifp, "Level Set Initialize", input, '=')) == 1) {
      ...
      /* OK.  Let us put the Variables initialized index by LS on the end of the original Var_init
       * array. This is why we add Num_Var_Init to the ls->Num_Var_Init to define index
       */
      Var_init[index].var = variable_string_to_int(input, "Initialize Keyword Error");

      if (fscanf(ifp, "%d %lf %lf", &(Var_init[index].ktype), &(Var_init[index].init_val_minus),
                 &(Var_init[index].init_val_plus)) != 3) {
        GOMA_EH(GOMA_ERROR, "Need one int and two floats on LS initialization card \n");
      }

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %s %d %.4g %.4g", "Level Set Initialize",
               input, Var_init[index].ktype, Var_init[index].init_val_minus,
               Var_init[index].init_val_plus);
      ECHO(echo_string, echo_file);
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%lf", &(ls->Length_Scale)) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Level Set Length Scale");
      }
      if (fabs(ls->Length_Scale) == 0.0)
        GOMA_WH(-1, "Possible Syntax error.  Level set length scale is zero.");

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", input, ls->Length_Scale);
    } else {
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, " (%s = %f) %s", "Level Set Length Scale",
               ls->Length_Scale, default_string);
    }
    ...
    if (ls->adapt) {
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%lf", &(ls->adapt_width)) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading Level Set Adapt Width");
        }
        ...
        snprintf(echo_string, MAX_CHAR_ECHO_INPUT, " (%s = %f) %s", "Level Set Adapt Width",
                 ls->adapt_width, default_string);
      }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%lf", &(ls->adapt_inner_size)) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading Level Set Adapt Inner Size");
        }
        ...
        snprintf(echo_string, MAX_CHAR_ECHO_INPUT, " (%s = %f) %s", "Level Set Adapt Inner Size",
                 ls->adapt_inner_size, default_string);
      }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%lf", &(ls->adapt_outer_size)) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading Level Set Adapt Outer Size");
        }

        snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", input, ls->adapt_outer_size);
      } else {
        snprintf(echo_string, MAX_CHAR_ECHO_INPUT, " (%s = %f) %s", "Level Set Adapt Outer Size",
                 ls->adapt_outer_size, default_string);
      }
      if (fabs(ls->adapt_outer_size) <= 0.0)
        GOMA_EH(GOMA_ERROR, "Possible Syntax error.  Level set adapt outer size <= 0");
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%d", &(ls->adapt_freq)) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading Level Set Adapt Frequency");
        }
        if (ls->adapt_freq <= 0)
          GOMA_EH(GOMA_ERROR, "Syntax error.  Level set adapt freq <= 0");
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%s", input) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Level Set Initialization string");
      }
      ...
      if (strcmp(input, "Projection") == 0) {
        ...
        snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Level Set Initialization Method",
                 input);
        ...
        if (fscanf(ifp, "%s %d %s %d", input, &(s->ns_id), input, &(s->PosEB_id)) != 4) {
          GOMA_EH(
              GOMA_ERROR,
              "Nodeset Initialization requires syntax:  Nodeset NS node_side_id EB e_block_id \n");
        }

        SPF(endofstring(echo_string), " NS %d EB %d", s->ns_id, s->PosEB_id);
        ECHO(echo_string, echo_file);
        ...
        snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "Level Set Initialization Method = Surfaces %d",
                 num_surf);
        ECHO(echo_string, echo_file);
    ...
    for (i = 0; i < 6; i++)
      ls->Periodic_Plane_Loc[i] = 0.;
    iread = look_for_optional(ifp, "Level Set Periodic Planes", input, '=');
    if (iread == 1) {
      if (fscanf(ifp, "%lf %lf %lf %lf %lf %lf", &(ls->Periodic_Plane_Loc[0]),
                 &(ls->Periodic_Plane_Loc[1]), &(ls->Periodic_Plane_Loc[2]),
                 &(ls->Periodic_Plane_Loc[3]), &(ls->Periodic_Plane_Loc[4]),
                 &(ls->Periodic_Plane_Loc[5])) != 6) {
        GOMA_EH(GOMA_ERROR, "error reading Level Set Periodic Planes");
      }
      ls->Periodic_Planes = TRUE;

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT,
               "Level Set Periodic Planes = %.4g %.4g %.4g %.4g %.4g %.4g",
               ls->Periodic_Plane_Loc[0], ls->Periodic_Plane_Loc[1], ls->Periodic_Plane_Loc[2],
               ls->Periodic_Plane_Loc[2], ls->Periodic_Plane_Loc[3], ls->Periodic_Plane_Loc[3]);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      ...
      if (ls->Control_Width > 10.0)
        GOMA_WH(-1, "That's an awfully large Level Set Control Width.\n");

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", input, ls->Control_Width);
      ECHO(echo_string, echo_file);

    } else
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, " (%s = %f) %s", "Level Set Control Width",
               ls->Control_Width, default_string);
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%s", input) != 1) {
        GOMA_EH(GOMA_ERROR, "Expecting string for Level Set Timestep Control");
      }
      stringup(input);
      if (!strcmp(input, "YES") || !strcmp(input, "ON"))
        tran->use_var_norm[8] = 1;

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Level Set Timestep Control", input);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%lf", &(ls->Renorm_Tolerance)) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Level Set Renormalization tolerance");
      }
      if (ls->Renorm_Tolerance > 1.0)
        GOMA_WH(-1, "That's an awfully large Level Set Renormalization Tolerance.\n");

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Level Set Renormalization Tolerance",
               ls->Renorm_Tolerance);
    } else
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, " (%s = %f) ",
               "Level Set Renormalization Tolerance", ls->Renorm_Tolerance);
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%s", input) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Level Set Renormalization string");
      }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%s", input) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Ignore Level Set Dependencies");
      }
      strip(input);
      stringup(input);
    ...
    if (iread == 1) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Level Set Output Filename", input);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Level Set Renormalization Frequency",
               ls->Renorm_Freq);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Level Set Renormalization Subcycles",
               ls->SubcyclesAfterRenorm);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%s", input) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Force Initial Level Set Renormalization flag");
      }
      strip(input);
      stringup(input);
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat,
               "Force Initial Level Set Renormalization", input);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%lf", &(ls->Initial_LS_Displacement)) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Initial Level Set Displacement");
      }

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Initial Level Set Displacement",
               ls->Initial_LS_Displacement);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%s", input) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Level Set Reconstruction Method");
      }
      strip(input);
      stringup(input);
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Level Set Reconstruction Method",
               input);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%s", input) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Level Set Contact Extension flag");
      }
      strip(input);
      stringup(input);
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Level Set Contact Extension", input);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      ...
      if ((strcmp(input, "ON") == 0) || (strcmp(input, "YES") == 0)) {
        ...
        if (ls->Init_Method != SURFACES)
          GOMA_EH(GOMA_ERROR, "Must define Level Set Initialization method to be SURFACES or "
                              "NODESET for slave surfaces");
      }

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Level Set Slave Surface", input);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Level Set Fluid Solid", input);
      ECHO(echo_string, echo_file);
    }
    ...
    if (ls->Fluid_Solid) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Level Set Solid Sign", ls->Solid_Sign);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      ...
      if (strcmp(input, "GRID_SEARCH") == 0) {
        int err;
        GOMA_EH(GOMA_ERROR,
                "The Level Set Search Option : GRID_SEARCH is not functioning at this time.\n");
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%d", &(ls->Integration_Depth)) != 1) {
        GOMA_EH(
            GOMA_ERROR,
            "Subgrid Integration Depth for Level Set needs a single positive integer parameter.");
      }
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Level Set Subgrid Integration Depth",
               ls->Integration_Depth);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      ...
      if ((strcmp(input, "ON") == 0) || (strcmp(input, "YES") == 0)) {
        ...
        if (ls->Integration_Depth > 0)
          GOMA_EH(GOMA_ERROR,
                  "Combination of Subgrid and Subelement integration is not supported.");
        if (ls->Length_Scale > 0.)
          GOMA_EH(GOMA_ERROR, "I don't think it makes sense to have subelement integration and "
                              "non-zero length scale.");
      }

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Level Set Subelement Integration",
               input);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      ...
      if ((strcmp(input, "ON") == 0) || (strcmp(input, "YES") == 0)) {
        ...
        if (ls->Length_Scale > 0.)
          GOMA_EH(GOMA_ERROR, "Currently, Adaptive integration applies only to sharp interfaces "
                              "(zero LS length scale).");
        if (ls->SubElemIntegration)
          GOMA_EH(GOMA_ERROR,
                  "Combination of Subelement and Adaptive integration is not supported.");
      }
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Level Set Adaptive Integration", input);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%d", &(ls->Adaptive_Order)) != 1) {
        GOMA_EH(
            GOMA_ERROR,
            "Adaptive Integration Order for Level Set needs a single positive integer parameter.");
      }
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Level Set Adaptive Order",
               ls->Adaptive_Order);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Overlap Quadrature Points",
               ls->CrossMeshQuadPoints);
      ECHO(echo_string, echo_file);
    }

    /*
     *  This flag is set so that the PSPP stabilization parameter will be weighted by 1.0 - delta *
     * alpha at the interface This will filter the PSPP stabilization at the interface region and
     * improve mass conservatino At the same time, convergence and matrix condition may be adversely
     * affected
     */
    ...
    if (iread == 1) {
      ...
      if ((strcmp(input, "ON") == 0) || (strcmp(input, "YES") == 0)) {
        ls->PSPP_filter = TRUE;
        snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %s", "Level Set PSPP filtering", "yes");
        ECHO(echo_string, echo_file);

      } else {
        snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %s", "Level Set PSPP filtering", "no");
        ECHO(echo_string, echo_file);
      }
    }
    ...
    if (iread == 1) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %s",
               "Level Set Semi-Implicit Time Integration", input);
      ECHO(echo_string, echo_file);
    }
  ...
  if (iread == 1) {
    ...
    if (i > 0) {
      ...
      for (i = 0; i < pfd->num_phase_funcs; i++) {
        ...
        if (ls != NULL)
          pfd->ls[i]->CrossMeshQuadPoints = ls->CrossMeshQuadPoints;
        else
          pfd->ls[i]->CrossMeshQuadPoints = 0.;
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Number of phase functions",
               pfd->num_phase_funcs);
      ECHO(echo_string, echo_file);
    } else {
      ECHO("\n***Phase Function Section***\n", echo_file);
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Number of phase functions", i);
      ECHO(echo_string, echo_file);
    }
  }
  ...
  if (pfd != NULL) {
    ...
    if (iread == 1) {
...
#ifdef PHASE_COUPLED_FILL
        GOMA_EH(GOMA_ERROR,
                "Phase function slave surfaces not currently supported for PHASE_COUPLED_FILL.");
#endif
        for (i = 0; i < pfd->num_phase_funcs; i++) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Phase Function Slave Surface", input);
      ECHO(echo_string, echo_file);
    }
    ...
    if (pfd != NULL) {
      if (pfd->ls[0]->Evolution != LS_EVOLVE_SLAVE) {
        ...
        GOMA_EH(GOMA_ERROR, "Must have PHASE_COUPLED_FILL compile flag for phase-field functions "
                            "that are not SLAVED");
#endif
      }
    }
    ...
    if (iread == 1) {
      for (i = 0; i < pfd->num_phase_funcs; i++) {
        ...
        if (fscanf(ifp, "%s", input) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading Phase Function Initialization string");
        }
        ...
        if (strcmp(input, "Projection") == 0) {
          ...
          snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat,
                   "Phase Function Initialization Method", input);
          ...
          if (fscanf(ifp, "%s %d %s %d", input, &(s->ns_id), input, &(s->PosEB_id)) != 4) {
            GOMA_EH(GOMA_ERROR, "Nodeset Initialization requires syntax:  Nodeset NS node_side_id "
                                "EB e_block_id \n");
          }
          ...
          if (fscanf(ifp, "%d", &num_surf) != 1) {
            GOMA_EH(GOMA_ERROR, "Surfaces initialization method for phase function needs number of "
                                "surfaces specified.");
          }

          snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %s %d",
                   "Phase Function Initialization Method", input, num_surf);
          ECHO(echo_string, echo_file);
    ...
    if (iread == 1) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Phase Function Length Scale",
               length_scale);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      ...
      if (fscanf(ifp, "%lf", &tolerance) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Phase Function Renormalization tolerance");
      }
      if (tolerance > 1.0)
        GOMA_WH(-1, "That's an awfully large Level Set Renormalization Tolerance.\n");
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g",
               "Phase Function Renormalization Tolerance", tolerance);
      ECHO(echo_string, echo_file);
    }
    ...
    if (iread == 1) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Phase Function Renormalization Method",
               input);
      ECHO(echo_string, echo_file);
    }
  }
  ...
  if (iread == 1) {
    ...
    if (pfd->Constraint_Method == LAGR_MULT) {
      ...
      if (augc == NULL) {
        ...
        GOMA_EH(GOMA_ERROR,
                "LM phase function constraint method must be first augmenting condition.\n");
      }
      ...
      if (fscanf(ifp, "%d %lf", &(augc[nAC - 1].MTID), &(augc[nAC - 1].CONSTV)) != 2) {
        GOMA_EH(GOMA_ERROR,
                "LM constraint type for phase function needs integer and double arguments.\n");
      }

      SPF(endofstring(echo_string), " %d %.4g", augc[nAC - 1].MTID, augc[nAC - 1].CONSTV);
      ECHO(echo_string, echo_file);
    }
  }
 ...
 *              into "sections".
 */
void rd_elem_quality_specs(FILE *ifp, char *input) {
  ...
  eqm->do_jac = FALSE;
  eqm->wt_jac = 0.0;
  iread = look_for_optional(ifp, "Jacobian quality weight", input, '=');
  if (iread == 1) {
    nEQM++;
    eqm->do_jac = TRUE;
    if (fscanf(ifp, "%lf", &(eqm->wt_jac)) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Jacobian quality weight");
    }
  }

  eqm->do_vol = FALSE;
  eqm->wt_vol = 0.0;
  iread = look_for_optional(ifp, "Volume change quality weight", input, '=');
  if (iread == 1) {
    nEQM++;
    eqm->do_vol = TRUE;
    if (fscanf(ifp, "%lf", &(eqm->wt_vol)) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Volume change quality weight");
    }
  }

  eqm->do_ang = FALSE;
  eqm->wt_ang = 0.0;
  iread = look_for_optional(ifp, "Angle quality weight", input, '=');
  if (iread == 1) {
    nEQM++;
    eqm->do_ang = TRUE;
    if (fscanf(ifp, "%lf", &(eqm->wt_ang)) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Angle quality weight");
    }
  }

  eqm->do_tri = FALSE;
  eqm->wt_tri = 0.0;
  iread = look_for_optional(ifp, "Triangle quality weight", input, '=');
  if (iread == 1) {
    nEQM++;
    eqm->do_tri = TRUE;
    if (fscanf(ifp, "%lf", &(eqm->wt_tri)) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Triangle quality weight");
    }
  }

  eqm->eq_tol = 0.0;
  iread = look_for_optional(ifp, "Element quality tolerance", input, '=');
  if (iread == 1) {
    eqm->do_jac = TRUE;
    if (fscanf(ifp, "%lf", &(eqm->eq_tol)) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Element quality tolerance");
    }
  }
  ...
  if (iread == 1) {
    ...
    if (strcmp(input, "jac") == 0) {
      ...
      GOMA_EH(GOMA_ERROR, "error reading Element quality tolerance type");
    }
  } else {
    eqm->tol_type = EQM_MIN;
  }
...
void rd_track_specs(FILE *ifp, char *input) {
  char *yo;
  ...
  if (iread == 1) {
    ...
    if (strcmp(input, "none") == 0) {
      ...
      GOMA_EH(GOMA_ERROR, "unknown Continuation option");
    }
    ...
    if (strcmp(input, "none") == 0) {
      ...
      GOMA_EH(GOMA_ERROR, "Unknown ContType option (one of BC, MT, AC, UM, AN or UF)");
    }
    ...
    if (iread == 1) {
      if (fscanf(ifp, "%d", &BdyCondID) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Boundary condition ID");
      }
    ...
    if (fscanf(ifp, "%d", &DataFltID) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Boundary condition data float tag id");
    }
    ...
    if (fscanf(ifp, "%d", &MatID) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Material id");
    }
    ...
    if (fscanf(ifp, "%d", &MatPropID) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Material property tag id");
    }
    ...
    if (fscanf(ifp, "%d", &MatPropSIID) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Material property tag subindex id");
    }
    ...
    if (fscanf(ifp, "%le", &BegParameterValue) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading BegParameterValue");
    }
    ...
    if (fscanf(ifp, "%le", &EndParameterValue) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading EndParameterValue");
    }
    ...
    if (fscanf(ifp, "%le", &Delta_s0) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading delta_s");
    }
    ...
    if (fscanf(ifp, "%d", &MaxPathSteps) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading maximum number of path steps");
    }
    ...
    if (fscanf(ifp, "%le", &Delta_s_min) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading minimum path step");
    }
    ...
    if (fscanf(ifp, "%le", &Delta_s_max) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Maximum path step");
    }
    ...
    if (fscanf(ifp, "%d", &print_freq) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Continuation Printing Frequency");
    }
    ...
    if (Num_Proc > 1) {
      ...
      if (iread == 1) {
        ...
        snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Continuation Fix Frequency",
                 cont->fix_freq);
        ECHO(echo_string, echo_file);
      }
    }
    ...
    if (Continuation == LOCA) {
        ...
        if (strcmp(input, "ss") == 0) {
          ...
          if (iread == 1) {
            if (fscanf(ifp, "%d", &(loca_in->Cont_Order)) != 1) {
              GOMA_EH(GOMA_ERROR, "error reading continuation order");
            }
          ...
          GOMA_EH(GOMA_ERROR, "Invalid LOCA method!");
        }
      }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%lf", &(loca_in->StepAggr)) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading step control aggressiveness");
        }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%lf", &(loca_in->perturb)) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading Perturbation magnitude");
        }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%d", &(loca_in->debug)) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading LOCA print level");
        }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%lf", &(loca_in->DpDs2)) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading ALC Desired solution fraction");
        }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%lf", &(loca_in->DpDsHi)) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading ALC Max. parameter sensitivity");
        }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%lf", &(loca_in->Texp)) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading ALC Tangent factor exponent");
        }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%lf", &(loca_in->MaxTS)) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading ALC Tangent factor step limit");
        }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%d", &nCC) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading Number of continuation conditions");
        }
        ...
        if (nCC < 1 && nCC != -2) {
          Continuation = ALC_NONE;
          snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s)",
                   "No continuation conditions specified");
          return;
        } else
      ...
      if (nCC != -2) {
        ...
        if (nCC > 1 && cont->upType == 5) {
          snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s)",
                   "UF continuation overrides CC cards!");
          ECHO(echo_string, echo_file);
          nCC = 1;
        }
        ...
        if (nCC > 1) {
          for (iCC = 1; iCC < nCC; iCC++) {
            ...
            case 1: /* BC */
              cpcc[iCC].BCID = id1;
              cpcc[iCC].DFID = id2;
              snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t( %3d. BC: BCID=%3d DFID=%5d)", iCC + 1,
            ...
            case 2: /* MT */
              cpcc[iCC].MTID = id1;
              cpcc[iCC].MPID = id2;
              snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t( %3d. MT: MTID=%3d MPID=%5d)", iCC + 1,
            ...
            case 3: /* AC */
              cpcc[iCC].BCID = id1;
              cpcc[iCC].DFID = id2;
              snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t( %3d. AC: ACID=%3d DFID=%5d)", iCC + 1,
                       cpcc[iCC].BCID, cpcc[iCC].DFID);
              break;
            ...
            case 4: /* UM */
              ...
              cpcc[iCC].MPID = id2;
              cpcc[iCC].MDID = id3;
              snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t( %3d. UM: MTID=%3d MPID=%5d MDID=%3d)",
    ...
    if (Continuation == LOCA &&
      ...
      if (strcmp(input, "none") == 0) {
        ...
        GOMA_EH(GOMA_ERROR, "unknown TP ContType option (one of BC, MT, AC, UM, AN, or UF)");
      }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%d", &TPBdyCondID) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading TP Boundary condition ID");
        }
      ...
      if (fscanf(ifp, "%d", &TPDataFltID) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading TP BC data float tag id");
      }
      ...
      if (fscanf(ifp, "%d", &TPMatID) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading TP parameter material id");
      }
      ...
      if (fscanf(ifp, "%d", &TPMatPropID) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading TP parameter material property tag id");
      }
      ...
      if (fscanf(ifp, "%d", &TPMatPropSIID) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading TP Material property tag subindex id");
      }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%lf", &(loca_in->TPGuess)) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading initial guess of TP parameter");
        }
      }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%lf", &(loca_in->TPFinal)) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading TP parameter final value");
        }
      }
      ...
      if (iread == 1) {
        if (fscanf(ifp, "%d", &nTC) != 1) {
          GOMA_EH(GOMA_ERROR, "error reading Number of TP continuation conditions");
        }
        ...
        if (nTC < 1) {
          Continuation = ALC_NONE;
          snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s)",
                   "No TP continuation conditions specified");
          ECHO(echo_string, echo_file);
          return;
        }
      }
      ...
      if (nTC > 1) {
        ...
        for (iTC = 1; iTC < nTC; iTC++) {
          ...
          case 1:
            ...
            tpcc[iTC].BCID = id1;
            tpcc[iTC].DFID = id2;
            snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t( %3d. BC: BCID=%3d DFID=%5d)", iTC + 1,
          ...
          case 2:
            ...
            tpcc[iTC].MTID = id1;
            tpcc[iTC].MPID = id2;
            snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t( %3d. MT: MTID=%3d MPID=%5d)", iTC + 1,
          ...
          case 3:
            ...
            tpcc[iTC].BCID = id1;
            tpcc[iTC].DFID = id2;
            snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t( %3d. AC: ACID=%3d DFID=%5d)", iTC + 1,
                     tpcc[iTC].BCID, tpcc[iTC].DFID);
            break;
          ...
          case 4:
            ...
            tpcc[iTC].MPID = id2;
            tpcc[iTC].MDID = id3;
            snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t( %3d. MT: MTID=%3d MPID=%5d MDID=%3d)",
...
void rd_hunt_specs(FILE *ifp, char *input) {
  int iHC;
  ...
  if ((Continuation != HUN_ZEROTH) && (Continuation != HUN_FIRST) && (Continuation != LOCA)) {
    nHC = 0;
    return;
  }
  ...
  if (fscanf(ifp, "%d", &nHC) != 1) {
    GOMA_EH(GOMA_ERROR, "error reading Number of hunting conditions");
  }
  ...
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", input, nHC);
  ECHO(echo_string, echo_file);
  ...
  if (nHC == 0) {
    Continuation = -1;
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s)",
             "No hunting - reverting to no continuation");
    ECHO(echo_string, echo_file);
    return;
  }
  ...
  if (Debug_Flag && ProcID == 0) {
    printf("%s:\tallocated %d copies of the %lu sized\n", yo, nHC,
           (long unsigned int)sizeof(struct HC_Information));
  ...
  for (iHC = 0; iHC < nHC; iHC++) {
    ...
    case 1:
      /* AC */
      /* BC */
      if (fscanf(ifp, "%d %d %d %le %le %le %le %le", &hunt[iHC].BCID, &hunt[iHC].DFID,
                 &hunt[iHC].ramp, &hunt[iHC].BegParameterValue, &hunt[iHC].EndParameterValue,
                 &hunt[iHC].Delta_s0, &hunt[iHC].Delta_s_min, &hunt[iHC].Delta_s_max) != 8) {
      ...
      SPF(endofstring(echo_string), " %d %d %d %.4g %.4g %.4g %.4g %.4g", hunt[iHC].BCID,
          hunt[iHC].DFID, hunt[iHC].ramp, hunt[iHC].BegParameterValue, hunt[iHC].EndParameterValue,
          hunt[iHC].Delta_s0, hunt[iHC].Delta_s_min, hunt[iHC].Delta_s_max);
      SPF(endofstring(echo_string), "\n\t( %3d. BC: BCID=%3d DFID=%5d)", iHC + 1, hunt[iHC].BCID,
          hunt[iHC].DFID);
      break;

    case 2:
      /* MT */
      if (fscanf(ifp, "%d %d %d %le %le %le %le %le", &hunt[iHC].MTID, &hunt[iHC].MPID,
                 &hunt[iHC].ramp, &hunt[iHC].BegParameterValue, &hunt[iHC].EndParameterValue,
                 &hunt[iHC].Delta_s0, &hunt[iHC].Delta_s_min, &hunt[iHC].Delta_s_max) != 8) {
      ...
      SPF(endofstring(echo_string), " %d %d %d %.4g %.4g %.4g %.4g %.4g", hunt[iHC].MTID,
          hunt[iHC].MPID, hunt[iHC].ramp, hunt[iHC].BegParameterValue, hunt[iHC].EndParameterValue,
          hunt[iHC].Delta_s0, hunt[iHC].Delta_s_min, hunt[iHC].Delta_s_max);
      SPF(endofstring(echo_string), "\n\t( %3d. MT: MTID=%3d MPID=%5d)", iHC + 1, hunt[iHC].MTID,
          hunt[iHC].MPID);
      hunt[iHC].MTID--;
      break;
    ...
    case 4:
      /* UM */
      if (fscanf(ifp, "%d %d %d %d %le %le %le %le %le", &hunt[iHC].MTID, &hunt[iHC].MPID,
                 &hunt[iHC].MDID, &hunt[iHC].ramp, &hunt[iHC].BegParameterValue,
                 &hunt[iHC].EndParameterValue, &hunt[iHC].Delta_s0, &hunt[iHC].Delta_s_min,
                 &hunt[iHC].Delta_s_max) != 9) {
        fprintf(stderr, "%s:\tError reading hunt[%d].MTID hunt[%d].MPID hunt[%d].MDID\n", yo, iHC,
                iHC, iHC);
        fprintf(stderr,
                "%s:\tRecall Format: HC = MTID MPID MDID RAMP from to step step_min step_max\n",
                yo);
        exit(-1);
      }
      SPF(endofstring(echo_string), " %d %d %d %d %.4g %.4g %.4g %.4g %.4g", hunt[iHC].MTID,
          hunt[iHC].MPID, hunt[iHC].MDID, hunt[iHC].ramp, hunt[iHC].BegParameterValue,
          hunt[iHC].EndParameterValue, hunt[iHC].Delta_s0, hunt[iHC].Delta_s_min,
          hunt[iHC].Delta_s_max);
      SPF(endofstring(echo_string), "\n\t( %3d. MT: MTID=%3d MPID=%5d MDID=%3d)", iHC + 1,
          hunt[iHC].MTID, hunt[iHC].MPID, hunt[iHC].MDID);
      hunt[iHC].MTID--;
      break;
    }
  ...
  if (Continuation == LOCA) {
    ...
    nCC = nHC;
    cpcc = (struct Continuation_Conditions *)array_alloc(1, nCC,
                                                         sizeof(struct Continuation_Conditions));
    if (Debug_Flag && ProcID == 0) {
...
void rd_ac_specs(FILE *ifp, char *input) {
  char err_msg[MAX_CHAR_IN_INPUT];
  char echo_string[MAX_CHAR_ECHO_INPUT] = "\0";
  char *echo_file = Echo_Input_File;
  ...
  if (fscanf(ifp, "%d", &nAC) != 1) {
    GOMA_EH(GOMA_ERROR, "error reading Number of augmenting conditions");
  }
  ...
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Number of augmenting conditions", nAC);
  ECHO(echo_string, echo_file);
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Augmenting Conditions Initial Guess",
             input);
    ECHO(echo_string, echo_file);
  }
  ...
  if (Debug_Flag && ProcID == 0) {
    printf("%s:\tallocated %d copies of the %lu sized\n", yo, nAC,
           (long unsigned int)sizeof(struct AC_Information));
  ...
  for (iAC = 0; iAC < nAC1; iAC++) /* Begin loop over ACs */
  {
    ...
    if (fscanf(ifp, "%80s", input) != 1) {
      fprintf(stderr,
              "%s:\tError reading AC[iAC].Set_Type (one of BC, MT, VC, LSV, FC, LM, or OV)\n", yo);
      exit(-1);
    }
    ...
    } else if (!strcmp(input, "MT") || !strcmp(input, "USERMAT")) {
      augc[iAC].Type = AC_USERMAT;
    } else if (!strcmp(input, "VC") || !strcmp(input, "VOLUME")) {
    ...
    } else if (!strcmp(input, "FC") || !strcmp(input, "FLUX")) {
      augc[iAC].Type = AC_FLUX;
    } else if (!strcmp(input, "LM") || !strcmp(input, "LGR")) {
      augc[iAC].Type = AC_LGRM;
    } else if (!strcmp(input, "OV") || !strcmp(input, "OVERLAP")) {
      augc[iAC].Type = AC_OVERLAP;
    } else if (!strcmp(input, "PBC") || !strcmp(input, "PERIODIC")) {
      augc[iAC].Type = AC_PERIODIC;
    } else if (!strcmp(input, "FCM") || !strcmp(input, "FLUXMAT")) {
    ...
    case AC_LS_VEL: /* LSV */
      ...
      if (strcmp(input, "VX") == 0) {
        ...
        GOMA_EH(
            GOMA_ERROR,
            "Must specify the velocity component for LS_VEL augmenting condition as VX, VY or VZ.");
      }
    ...
    case AC_FLUX_MAT: /* FC */
      ...
      if (augc[iAC].Type == AC_FLUX) {
        if (fscanf(ifp, "%d %d %d %s", &augc[iAC].MTID, &augc[iAC].BCID, &augc[iAC].DFID, input) !=
            4) {
          GOMA_EH(GOMA_ERROR, "Error in syntax for flux-type augmenting condition");
        }

        SPF(endofstring(echo_string), " %d %d %d %s", augc[iAC].MTID, augc[iAC].BCID,
            augc[iAC].DFID, input);
        ...
        if (fscanf(ifp, "%d %d %d %s", &augc[iAC].MTID, &augc[iAC].MPID, &augc[iAC].MDID, input) !=
            4) {
          fprintf(stderr, "%s:\tError reading augc[%d].MTID augc[%d].MPID augc[%d].MDID\n", yo, iAC,
                  iAC, iAC);
          exit(-1);
        }
        SPF(endofstring(echo_string), " %d %d %d %s", augc[iAC].MTID, augc[iAC].MPID,
            augc[iAC].MDID, input);
      }
      ...
      case SPECIES_FLUX_REVOLUTION:
        SPF(endofstring(echo_string), " %d %d %.4g", augc[iAC].SSID, augc[iAC].COMPID,
            augc[iAC].CONSTV);
        break;
      case CURRENT_FICKIAN:
        SPF(endofstring(echo_string), " %d %d %.4g %.4g", augc[iAC].SSID, augc[iAC].COMPID,
            augc[iAC].CONSTV, z[augc[iAC].COMPID]);
        break;
      default:
    ...
    case AC_LGRM:
      ...
      if (fscanf(ifp, "%d %d %s %lf", &augc[iAC].MTID, &augc[iAC].SSID, input, &augc[iAC].CONSTV) !=
          4) {
      ...
      SPF(endofstring(echo_string), " %d %d %s %.4g", augc[iAC].MTID, augc[iAC].SSID, input,
          augc[iAC].CONSTV);
    ...
    case AC_OVERLAP:
       ...
       */
      if (fscanf(ifp, "%d %d %d %d", &augc[iAC].SSID, &augc[iAC].solid_eb, &augc[iAC].fluid_eb,
                 &augc[iAC].lm_eb) < 4) {
      ...
      if (augc[iAC].solid_eb != augc[iAC].lm_eb && augc[iAC].fluid_eb != augc[iAC].lm_eb) {
      ...
      SPF(endofstring(echo_string), " %d %d %d %d", augc[iAC].SSID, augc[iAC].solid_eb,
...
void rd_solver_specs(FILE *ifp, char *input) {
  char *c;
  char echo_string[MAX_CHAR_ECHO_INPUT] = "\0";
  char *echo_file = Echo_Input_File;
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Number of Matrices",
  ...
  if (iread == 1) {
    ...
    if (!strcasecmp(input, "no")) {
      ...
      GOMA_EH(GOMA_ERROR, "Bad specification for Segregated Solve");
    }
  }
  ...
  if (strcmp(Matrix_Solver, "umf") == 0) {
    ...
    if (i == 4) {
      ...
      GOMA_EH(GOMA_ERROR, "Benner's frontal solver not functional on 64-bit builds. We suggest "
                          "using umf or at your own risk remove this error-handler statement");
    } else {
  ...
  if (strcmp(Matrix_Solver, "aztecoo") == 0) {
    ...
    if (iread == 1) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Format,
               default_string);
    }
    ECHO(echo_string, echo_file);
  } else {
  ...
  */
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%d", &UMFPACK_IDIM) != 1) {
      iread = -1;
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, " (%s = %d) %s", search_string, UMFPACK_IDIM,
               default_string);
      ECHO(echo_string, echo_file);
    } else {
  ...
  */
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%d", &UMFPACK_XDIM) != 1) {
      iread = -1;
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, " (%s = %d) %s", "UMF_XDIM", UMFPACK_XDIM,
               default_string);
      ECHO(echo_string, echo_file);
    } else
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, "stratimikos.xml",
             default_string);
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, search_string, Matrix_Preconditioner);
    ECHO(echo_string, echo_file);
  } else {
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Preconditioner,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Subdomain_Solver,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Scaling,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Residual_Norm_Type,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Output_Type,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Factorization_Reuse,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Graph_Fillin,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Factor_Overlap,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Overlap_Type,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Auxiliary_Vector,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Drop_Tolerance,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Polynomial_Order,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Reorder,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Factorization_Save,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_ILUT_Fill_Factor,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_RILU_Relax_Factor,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_BILU_Threshold,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Relative_Threshold,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Absolute_Threshold,
             default_string);
    ECHO(echo_string, echo_file);
  }
#endif
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Krylov_Subspace,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Orthogonalization,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, def_form, search_string, Matrix_Maximum_Iterations,
             default_string);
    ECHO(echo_string, echo_file);
  }
  ...
  if (fscanf(ifp, "%d", &Max_Newton_Steps) != 1) {
    GOMA_EH(GOMA_ERROR, "error reading Number of Newton Iterations");
  }
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Number of Newton Iterations",
           Max_Newton_Steps);
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%le %le", &convergence_rate_tolerance, &modified_newt_norm_tol) != 2) {
      GOMA_EH(GOMA_ERROR, "ERROR reading Modified Newton Tolerance card");
    } else {
      if (convergence_rate_tolerance != 0. && modified_newt_norm_tol != 0.) {
        modified_newton = TRUE;
      }
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g %.4g", "Modified Newton Tolerance",
               convergence_rate_tolerance, modified_newt_norm_tol);
      ECHO(echo_string, echo_file);
    }
  } else {
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%d", &Time_Jacobian_Reformation_stride) != 1) {
      GOMA_EH(GOMA_ERROR, "ERROR reading Jacobian Reform Stride card");
    } else {
      if (Time_Jacobian_Reformation_stride > 1)
        modified_newton = TRUE;
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Jacobian Reform Time Stride",
               Time_Jacobian_Reformation_stride);
      ECHO(echo_string, echo_file);
    }
  } else {
  ...
  while ((iread = look_forward_optional(ifp, "Variable Type Newton correction factor", input,
                                        '=')) == 1) {
    int var;
    var = -1;
    ...
    if (var == -1) {
      GOMA_EH(GOMA_ERROR,
              "Could not identify variable type for Variable Type Newton Correction Factor!");
    }

    if (fscanf(ifp, "%le", &var_damp[var]) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading value of variable type Newton correction factor");
    }
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %s %.4g",
             "Variable Type Newton correction factor", Var_Name[k].name1, var_damp[var]);
    ECHO(echo_string, echo_file);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Normalized Correction Tolerance",
             Epsilon[0][2]);
    ECHO(echo_string, echo_file);
  } else {
    Epsilon[0][2] = 1.0e+10;
  ...
  **/
  ...
  if (iread == 1) {
    ...
    if (fscanf(ifp, "%d", &Solver_Output_Format) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Solver Format");
    }
  ...
  if (iread == 1) {
    ...
    if (sscanf(input, "%le", &Epsilon[0][1]) != 1) {
      GOMA_EH(GOMA_ERROR, "Bad residual ratio (matrix convergence) tolerance.");
    }
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Residual Ratio Tolerance",
             Epsilon[0][1]);
    ECHO(echo_string, echo_file);
  ...
  if (iread == 1) {
    ...
    if (strcmp(input, "no") == 0) {
      ...
      GOMA_EH(
          -1,
          "invalid choice: %s, expected Pressure Stabilization yes, no, global or local, or pspp",
          input);
    }
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Pressure Stabilization", input);
    ECHO(echo_string, echo_file);
  } else {
    PSPG = 0;
    PSPP = 0;
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%le", &PS_scaling) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Pressure Stabilization Scaling");
    }
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g", "Pressure Stabilization Scaling",
             PS_scaling);
    ECHO(echo_string, echo_file);
  } else {
    PS_scaling = 0.;
  ...
  if (iread == 1) {
    ...
    if (strcmp(input, "no") == 0 || strcmp(input, "false") == 0) {
      ...
      GOMA_EH(GOMA_ERROR, "invalid choice: PSPG Advection Correction, yes (true) or no (false)");
    }
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "PSPG Advection Correction", input);
    ECHO(echo_string, echo_file);
  } else {
    upd->PSPG_advection_correction = false;
  ...
  if (iread == 1) {
    ...
    if (strcmp(input, "no") == 0 || strcmp(input, "false") == 0) {
      ...
      GOMA_EH(GOMA_ERROR, "invalid choice: DEVSS Traceless Gradient, yes (true) or no (false)");
    }
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "DEVSS Traceless Gradient", input);
    ECHO(echo_string, echo_file);
  } else {
    upd->devss_traceless_gradient = false;
  ...
  if (iread == 1) {
    ...
    if (strcmp(input, "no") == 0) {
      ...
      GOMA_EH(GOMA_ERROR, "invalid choice: Continuity Stabilization yes, local, or no");
    }
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Continuity Stabilization", input);
    ECHO(echo_string, echo_file);
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Number of Segregated Subcycles",
             upd->SegregatedSubcycles);
    ECHO(echo_string, echo_file);
  } else {
  ...
  if (iread == 1) {
    ...
    if (!strncmp(input, "no", 2))
      ...
      GOMA_EH(GOMA_ERROR, "Invalid choice: Linear Stability must be yes, no, file, 3D, or 3Dfile");

    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Linear Stability", input);
    ECHO(echo_string, echo_file);
  ...
  if (iread == 1) {
    ...
    if (fscanf(ifp, "%d %le %le", &filter_species_material_number, &c_min, &c_max) != 3) {
      GOMA_EH(GOMA_ERROR, "error reading Filter Concentration");
    }
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d %.4g %.4g", "Filter Concentration",
             filter_species_material_number, c_min, c_max);
    ECHO(echo_string, echo_file);
  } else {
  ...
  if (iread == 1) {
    ...
    if (strncmp(input, "no", 2) == 0) {
      ...
      GOMA_EH(GOMA_ERROR, "invalid choice for Disable Viscosity Sensitivities:must be yes or no");
    }
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "Disable Viscosity Sensitivities", input);
    ECHO(echo_string, echo_file);
  } else {
    Include_Visc_Sens = TRUE;
  }
  ...
  if (iread == 1) {
    ...
    if (strncmp(input, "no", 2) == 0) {
      ...
      GOMA_EH(GOMA_ERROR, "invalid choice for PETSc solve post proc : must be yes or no");
    }
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, eoformat, "PETSc solve post proc", input);
    ECHO(echo_string, echo_file);
  } else {
    upd->petsc_solve_post_proc = false;
  }
  ...
  /* look for optional flags specifying dependencies to ignore */
  {
    ...
    while ((iread = look_forward_optional(ifp, "Ignore Dependency", input, '=')) == 1) {
      ...
      for (eq = 0; eq < MAX_VARIABLE_TYPES && !eq_found; eq++) {
        if (!strcmp(input, EQ_Name[eq].name1) || !strcmp(input, EQ_Name[eq].name2)) {
          eq_found = TRUE;
        }
      }
      if (!eq_found) {
      ...
      if ((fscanf(ifp, "%2s", input) != 1) || (strcmp(input, "on") && strcmp(input, "ON"))) {
  ...
  if ((Num_Proc > 1) && (is_Solver_Serial == TRUE)) {
    GOMA_EH(GOMA_ERROR, "Your solver choice cannot be used for PARALLEL solutions");
  }
#endif
}
/* rd_solver_specs -- read input file for equation solver specifications */
...
void rd_eigen_specs(FILE *ifp, char *input) {
  int i;
  int iread;
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%d", &eigen->Eigen_NEV_WANT) != 1)
      iread = -1;
  }
  if (iread != 1) {
    eigen->Eigen_NEV_WANT = 10;
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %d)", "Eigen Number of modes",
             eigen->Eigen_NEV_WANT);
  } else {
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%d", &eigen->Eigen_Record_Modes) != 1)
      iread = -1;
  }
  if (iread != 1) {
    eigen->Eigen_Record_Modes = 0;
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %d)", "Eigen Record modes",
             eigen->Eigen_Record_Modes);
  } else {
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%d", &eigen->Eigen_Krylov_Subspace) != 1)
      iread = -1;
  }
  if (iread != 1) {
    eigen->Eigen_Krylov_Subspace = 30;
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %d)", "Eigen Size of Krylov subspace",
             eigen->Eigen_Krylov_Subspace);
  } else {
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%d", &eigen->Eigen_Maximum_Iterations) != 1)
      iread = -1;
  }
  if (iread != 1) {
    eigen->Eigen_Maximum_Iterations = 100;
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %d)", "Eigen Maximum Iterations",
             eigen->Eigen_Maximum_Iterations);
  } else {
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%d", &eigen->Eigen_Filter) != 1)
      iread = -1;
  }
  if (iread != 1) {
    eigen->Eigen_Filter = 2;
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %d)", "Eigen Number of Filter Steps",
             eigen->Eigen_Filter);
  } else {
  ...
  if (iread == 1) {
    ...
    if (strcmp(input, "no") == 0) {
      ...
      GOMA_EH(GOMA_ERROR, "invalid choice: Eigen Recycle must be yes or no");
    }
    SPF(endofstring(echo_string), " %s", input);
  } else {
    eigen->Eigen_Recycle = 0;
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %.4g)", "Eigen Tolerance",
             eigen->Eigen_Tolerance);
  }
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%le", &eigen->Eigen_IV_Wt) != 1) {
      GOMA_EH(GOMA_ERROR, "error reading Eigen Initial Vector Weight");
    }
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%le %le %le %le", &eigen->Eigen_Shifts[0], &eigen->Eigen_Shifts[1],
               &eigen->Eigen_Shifts[2], &eigen->Eigen_Shifts[3]) != 4) {
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %.4g)", "Eigen Cayley Sigma",
             eigen->Eigen_Cayley_Sigma);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %.4g)", "Eigen Cayley Mu",
             eigen->Eigen_Cayley_Mu);
  }
  ...
  if (iread == 1) {
    if (fscanf(ifp, "%d", &eigen->Eigen_Maximum_Outer_Iterations) != 1)
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", input,
               eigen->Eigen_Maximum_Outer_Iterations);
  }

  if (iread != 1) {
    eigen->Eigen_Maximum_Outer_Iterations = 1;
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %d)", "Eigen Maximum Outer Iterations",
             eigen->Eigen_Maximum_Outer_Iterations);
  }
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %.4g)", "Eigen SI tolerance parameter",
             eigen->Eigen_SI_Tol_Param);
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %.4g)", "Eigen Relative tolerance",
             eigen->Eigen_Relative_Tol);
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %.4g)", "Eigen Linear Solver tolerance",
             eigen->Eigen_Linear_Tol);
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %d)", "Eigenvalue output frequency",
             eigen->Eigen_Solve_Freq);
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %d)", "Eigenvector output frequency",
             eigen->Eigen_Write_Freq);
  ...
  if (iread == 1) {
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%s = %s)", "Eigenvector output file",
             eigen->Eigen_Output_File);
  }
  ...
  if (iread == 1) {
    ...
    if (LSA_wave_numbers[0] == 0.)
      GOMA_EH(GOMA_ERROR, "Cannot solve 3D of 2D for zero wave number");
    for (i = 1; i < LSA_number_wave_numbers; i++) {
      LSA_wave_numbers[i] = atof(strtok(NULL, " \t\n"));
      if (LSA_wave_numbers[i] == 0.)
        GOMA_EH(GOMA_ERROR, "cannot solve 3D of 2D for zero wave number");
    }

    SPF_DBL_VEC(endofstring(echo_string), LSA_number_wave_numbers, LSA_wave_numbers);
  } else if (Linear_Stability == LSA_3D_OF_2D || Linear_Stability == LSA_3D_OF_2D_SAVE)
    GOMA_EH(GOMA_ERROR,
            "3D stability of 2D flow requested, but missing \"Eigen Wave Numbers\" card");
...
void rd_matl_blk_specs(FILE *ifp, char *input) {
  char err_msg[MAX_CHAR_IN_INPUT];
  int err, mn, i, imtrx;
  int have_mesh_var;
  ...
  if (fscanf(ifp, "%d", &(upd->Num_Mat)) != 1) {
    fprintf(stderr, "Expected to read 1 int for \"Number of Materials\" - attempt to enumerate...");
    upd->Num_Mat = -1;
  }
  ...
  if (upd->Num_Mat > MAX_NUMBER_MATLS) {
    sprintf(err_msg, "Found num materials = %d > %d - boost MAX_NUMBER_MATLS in mm_mp_const.h",
            upd->Num_Mat, MAX_NUMBER_MATLS);
    GOMA_EH(GOMA_ERROR, err_msg);
  }

  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Number of Materials", upd->Num_Mat);
  ECHO(echo_string, echo_input_file);
  ...
  for (mn = 0; mn < upd->Num_Mat; mn++) { /* mn */
    ...
    if (efv->ev) {
      ...
      if ((strcmp(efv->name[0], "DMX") == 0 || strcmp(efv->name[0], "DISPLX") == 0) &&
          ...
          (have_mesh_var == 0) && (have_por_liq_pres_var == 1)) {
        efv->ev_porous_decouple = T_SOMETHING;
      } else if ((have_mesh_var == 0) && (have_por_liq_pres_var == 1)) {
        GOMA_EH(GOMA_ERROR, "You have por_liq_press equation and efv's, but not the right order. "
                            "Talk to PRS or RAR");
      }
    }
  ...
  for (imtrx = 0; imtrx < upd->Total_Num_Matrices; imtrx++) {
    if (upd->Total_Num_EQ[imtrx] > MAX_PROB_VAR) {
      fprintf(stderr, "%sTotal number of equations, %d, in Matrix, %d, exceeds MAX_PROB_VAR, %d\n",
              "input FATAL ERROR: ", upd->Total_Num_EQ[imtrx], (imtrx + 1), MAX_PROB_VAR);
      fprintf(stderr, "\tIncrease MAX_PROB_VAR to %d and rerun\n", upd->Total_Num_EQ[imtrx]);
 ...
 ******************************************************************/
{
  if (pd_ptr->e[mtrxNum][eqnNum]) {
    fprintf(stderr, "rd_eq_spec ERROR: eqnNum %d has already been activated.\n", eqnNum);
 ...
 ******************************************************************/
{
  if (pd_ptr->v[mtrxNum][varType]) {
    fprintf(stderr, "rd_eq_spec ERROR: variable %d has already been activated.\n", varType);
...
void rd_eq_specs(FILE *ifp, char *input, const int mn) {
  char err_msg[MAX_CHAR_IN_INPUT];
  ...
  if (fscanf(ifp, "%s", tscs) != 1) {
    GOMA_EH(GOMA_ERROR, "error reading Coordinate System");
  }
  ...
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %s", "Coordinate System", tscs);
  ECHO(echo_string, echo_file);
  ...
  if (strcasecmp(input, "isoparametric") == 0) {
    ...
    ECHO("\n\t(CAUTION WITH SUBPARAMETRIC: DON'T USE WITH LAGRANGIAN OR TOTAL_ALE MESH MOTIONS.  "
         "Also not good for GD_TABLE conditions.)",
         echo_file);
  } else {
    GOMA_EH(GOMA_ERROR, "invalid element mapping");
  }
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %s", "Element Mapping", input);
  ECHO(echo_string, echo_file);
  ...
  if (fscanf(ifp, "%s", tscs) != 1) {
    GOMA_EH(GOMA_ERROR, "error reading Mesh Motion");
  }
  ...
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %s", "Mesh Motion", tscs);
  ECHO(echo_string, echo_file);
  ...
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Number of bulk species", numBulkSpec);
  ECHO(echo_string, echo_file);
   ...
   *
   */
  retn = look_forward_optional(ifp, "Really meant number of bulk species equations in line above",
                               input, '=');
  if (retn == 1) {
  ...
  if (retn == 1) {
    ...
    if (nonDilute) {
      if (numBulkSpecEqn != pd_ptr->Num_Species_Eqn) {
        ...
        GOMA_EH(GOMA_ERROR,
                "\tNumber of bulk species equations is inconsistent with non-dilute assumption\n");
      }
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Number of bulk species equations",
               numBulkSpecEqn);
    }
   ...
   */
  if (pd_ptr->Num_Species_Eqn > 0) {
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(Number of bulk C/D equations  = %d)",
             pd_ptr->Num_Species_Eqn);
    ECHO(echo_string, echo_file);
    if (pd_ptr->Num_Species_Eqn != pd_ptr->Num_Species) {
      snprintf(
          echo_string, MAX_CHAR_ECHO_INPUT,
          "\t(Nondilute approximation: # species is not equal to number of species equations)\n");
  ...
  if (retn == 1) {
    ...
    if (upd->Species_Var_Type == 0)
      upd->Species_Var_Type = pd_ptr->Species_Var_Type;
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %s", "Default Material Species Type", input);
    ECHO(echo_string, echo_file);
  }
  ...
  if (err == -1) {
    vn_glob[mn]->modes = 0;
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Number of viscoelastic modes",
             vn_glob[mn]->modes);
    ECHO(echo_string, echo_file);
  } else {
    if (fscanf(ifp, "%d", &vn_glob[mn]->modes) != 1) {
      GOMA_EH(GOMA_ERROR, "Expected to read 1 int for \"Number of viscoelastic modes\"");
    }
  ...
  if (err == -1) {
    ...
    if (fscanf(ifp, "%d", &numMtrxLines) != 1) {
      GOMA_EH(GOMA_ERROR,
              "Expected to read 1 int for \"Number of Matrices\" - attempt to enumerate... ");
      numMtrxLines = -1;
    }
  }
  ...
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Number of Matrices", pd_ptr->Num_Matrices);
  ECHO(echo_string, echo_file);
  ...
  for (imtrx = 0; imtrx < numMtrxLines; imtrx++) {
    ...
    if (numMtrxLines == 1) {
      ...
      if (fscanf(ifp, "%d", &mtrx_index1) != 1) {
        GOMA_EH(GOMA_ERROR, "Expected to read 1 int for \"MATRIX\"  ");
      }

      mtrx_index0 = mtrx_index1 - 1;

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "MATRIX = %d", mtrx_index1);
      ECHO(echo_string, echo_file);

      if (look_forward_optional_until(ifp, "Disable time step control", "MATRIX", input, '=') ==
          1) {
        read_string(ifp, input, '\n');
        strip(input);
        if (strcmp(input, "yes") == 0) {
    ...
    if (iread == 1) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g matrix %d",
               "Normalized Residual Tolerance", Epsilon[imtrx][0], mtrx_index1);
      ECHO(echo_string, echo_file);
    }

    iread = look_forward_optional_until(ifp, "Matrix Subcycle Count", "MATRIX", input, '=');
    if (iread == 1) {
    ...
    iread =
        look_forward_optional_until(ifp, "Normalized Correction Tolerance", "MATRIX", input, '=');
    if (iread == 1) {
      if (fscanf(ifp, "%le", &Epsilon[imtrx][2]) != 1) {
        GOMA_EH(GOMA_ERROR, "error reading Normalized (Newton) Correction Tolerance");
      }
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %.4g matrix %d",
               "Normalized Correction Tolerance", Epsilon[imtrx][2], mtrx_index1);
      ECHO(echo_string, echo_file);
    }
    ...
    if (fscanf(ifp, "%d", &numEqnLines) != 1) {
      fprintf(stderr, "Expected to read 1 int for \"Number of EQ\" - attempt to enumerate...");
      numEqnLines = -1;
    }
    ...
    snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "%s = %d", "Number of EQ", numEqnLines);
    ECHO(echo_string, echo_file);

    if ((numEqnLines < 1 || numEqnLines > 25)) {
      GOMA_EH(GOMA_ERROR, "rd_eq_spec ERROR: Too many (>25) or too few eqn. lines");
    }
    ...
    for (ieqn = 0; ieqn < numEqnLines; ieqn++) { /*ieqn*/
      ...
      if (!strcasecmp(ts, "energy")) {
        ...
        if (ls == NULL)
          GOMA_EH(GOMA_ERROR, " NORMAL1 equation requires Level Set Interface tracking on.");
        ce = set_eqn(R_NORMAL1, mtrx_index0, pd_ptr);
      } else if (!strcasecmp(ts, "normal2")) {
        if (ls == NULL)
          GOMA_EH(GOMA_ERROR, " NORMAL2 equation requires Level Set Interface tracking on.");
        ce = set_eqn(R_NORMAL2, mtrx_index0, pd_ptr);
      } else if (!strcasecmp(ts, "normal3")) {
        if (ls == NULL)
          GOMA_EH(GOMA_ERROR, " NORMAL3 equation requires Level Set Interface tracking on.");
        ...
        ce = set_eqn(R_SHELL_SAT_1, mtrx_index0, pd_ptr);
        pd_ptr->Num_Porous_Shell_Eqn++;
      } else if (!strcasecmp(ts, "shell_sat_2")) {
        ce = set_eqn(R_SHELL_SAT_2, mtrx_index0, pd_ptr);
        pd_ptr->Num_Porous_Shell_Eqn++;
      } else if (!strcasecmp(ts, "shell_sat_3")) {
        ce = set_eqn(R_SHELL_SAT_3, mtrx_index0, pd_ptr);
        pd_ptr->Num_Porous_Shell_Eqn++;
        ...
        if (pd_ptr->e[mtrx_index0][R_POR_GAS_PRES] || pd_ptr->e[mtrx_index0][R_POR_SATURATION] ||
          ...
          fprintf(stderr, "%s%s\n",
                  "rd_eq_specs ERROR: ", "More than one porous media equation type");
        ...
        ce = set_eqn(R_POR_LIQ_PRES, mtrx_index0, pd_ptr);
        if (tran->theta != 0.0)
          GOMA_EH(GOMA_ERROR, "Please don't use Time step parameter of 0.5                         "
                              "          for porous media problems.  Use 0.0");
        ...
        if (!strcasecmp(ts, "species_unk")) {
          ...
          if (pd_ptr->Num_Species_Eqn == 0) {
            fprintf(stderr, "%s%s\n",
                    "rd_eq_specs ERROR: ", "Number of species unknowns is equal to zero");
          ...
          if (!interpret_int(ts + 12, &j)) {
            fprintf(stderr, "%s: Unable to decipher species eqn labeled: %s\n", yo, ts);
            exit(-1);
          }
          if (j < 0 || j > MAX_SPECIES_UNK_NUM) {
            fprintf(stderr, "%s: Species eqn %d is out of the permissible range\n", yo, j);
            exit(-1);
          }
      ...
      if (!strcasecmp(ts, "T")) {
        ...
        cv = set_var(SHELL_SAT_1, mtrx_index0, pd_ptr);
      } else if (!strcasecmp(ts, "SH_SAT_2")) {
        cv = set_var(SHELL_SAT_2, mtrx_index0, pd_ptr);
      } else if (!strcasecmp(ts, "SH_SAT_3")) {
        cv = set_var(SHELL_SAT_3, mtrx_index0, pd_ptr);
        ...
        if (!strcasecmp(ts, "Sp")) {
          ...
          if (!interpret_int(ts + 3, &j)) {
            fprintf(stderr, "%s: Unable to decipher species var labeled: %s\n", yo, ts);
            exit(-1);
          }
          if (j < 0 || j > MAX_SPECIES_UNK_NUM) {
            fprintf(stderr, "%s: Species var %d is out of the permissible range\n", yo, j);
            exit(-1);
          }
          if (eqnMult != 1) {
      ...
      if (fscanf(ifp, "%s", ts) != 1) {
        fprintf(stderr, "%s: problem reading interpolation function for variable.\n", yo);
        exit(-1);
      }
      ...
      if (in_list(pd_ptr->i[mtrx_index0][cv], 0, Num_Interpolations, Unique_Interpolations) == -1) {
      ...
      case R_SHELL_NORMAL3:
        ...
        if (fscanf(ifp, "%lf", &(pd_ptr->etm[mtrx_index0][ce][(LOG2_DIFFUSION)])) != 1) {
          sr = sprintf(err_msg, "Provide 1 equation term multiplier (dif) on %s in %s\n",
                       EQ_Name[ce].name1, pd_ptr->MaterialName);
          GOMA_EH(GOMA_ERROR, err_msg);
        }
        if (pd_ptr->etm[mtrx_index0][ce][(LOG2_DIFFUSION)] == 0.0) {
          sr = sprintf(err_msg,
                       "Equation term multiplier (dif) on %s in %s is equal to zero. Will get a "
                       "singular jacobian\n",
                       EQ_Name[ce].name1, pd_ptr->MaterialName);
          GOMA_EH(GOMA_ERROR, err_msg);
        }
      ...
      case R_PRESSURE:
        ...
        if (fscanf(ifp, "%lf %lf", &(pd_ptr->etm[mtrx_index0][ce][(LOG2_ADVECTION)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_SOURCE)])) != 2) {
        ...
        SPF(endofstring(echo_string), "\t %.4g %.4g",
            pd_ptr->etm[mtrx_index0][ce][(LOG2_ADVECTION)],
            pd_ptr->etm[mtrx_index0][ce][(LOG2_SOURCE)]);
        break;
      case R_GRADIENT11:
      ...
      case R_SHELL_CROSS_SHEAR:
        ...
        if (fscanf(ifp, "%lf %lf", &(pd_ptr->etm[mtrx_index0][ce][(LOG2_ADVECTION)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_SOURCE)])) != 2) {
          sr = sprintf(err_msg, "Provide 2 equation term multipliers (adv,src) on %s in %s",
                       EQ_Name[ce].name1, pd_ptr->MaterialName);
          GOMA_EH(GOMA_ERROR, err_msg);
        }
      ...
      case R_SHELL_DELTAH:
        if (fscanf(ifp, "%lf %lf", &(pd_ptr->etm[mtrx_index0][ce][(LOG2_MASS)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_DIVERGENCE)])) != 2) {
          sr = sprintf(err_msg, "Provide 2 equation term multipliers (mass,div) on %s in %s",
                       EQ_Name[ce].name1, pd_ptr->MaterialName);
          GOMA_EH(GOMA_ERROR, err_msg);
        }
      ...
      case R_CUR_STRAIN:
        if (fscanf(ifp, "%lf %lf", &(pd_ptr->etm[mtrx_index0][ce][(LOG2_MASS)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_SOURCE)])) != 2) {
          sr = sprintf(err_msg, "Provide 2 equation term multipliers (mass,src) on %s in %s",
                       EQ_Name[ce].name1, pd_ptr->MaterialName);
          GOMA_EH(GOMA_ERROR, err_msg);
        }
      ...
      case R_POR_SINK_MASS:
        ...
        break;
      case R_SHEAR_RATE:
        if (fscanf(ifp, "%lf %lf %lf", &(pd_ptr->etm[mtrx_index0][ce][(LOG2_ADVECTION)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_DIFFUSION)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_SOURCE)])) != 3) {
          sr = sprintf(err_msg, "Provide 3 equation term multipliers (adv,diff,src) on %s in %s",
                       EQ_Name[ce].name1, pd_ptr->MaterialName);
          GOMA_EH(GOMA_ERROR, err_msg);
        }
      ...
      case R_SHELL_SAT_3:
        if (fscanf(ifp, "%lf %lf %lf", &(pd_ptr->etm[mtrx_index0][ce][(LOG2_MASS)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_DIFFUSION)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_SOURCE)])) != 3) {
          sr = sprintf(err_msg, "Provide 3 equation term multipliers (mass,diff,src) on %s in %s",
                       EQ_Name[ce].name1, pd_ptr->MaterialName);
          GOMA_EH(GOMA_ERROR, err_msg);
        }
        SPF(endofstring(echo_string), "\t %.4g %.4g %.4g",
      ...
      case R_TFMP_MASS:
        if (fscanf(ifp, "%lf %lf %lf", &(pd_ptr->etm[mtrx_index0][ce][(LOG2_MASS)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_ADVECTION)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_DIFFUSION)])) != 3) {
          sr = sprintf(err_msg, "Provide 3 equation term multipliers (mass,adv,dif) on %s in %s",
                       EQ_Name[ce].name1, pd_ptr->MaterialName);
          GOMA_EH(GOMA_ERROR, err_msg);
        }
        SPF(endofstring(echo_string), "\t %.4g %.4g %.4g",
      ...
      case R_BOND_EVOLUTION:
        ...
        if (fscanf(ifp, "%lf %lf %lf  %lf", &(pd_ptr->etm[mtrx_index0][ce][(LOG2_MASS)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_ADVECTION)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_DIFFUSION)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_SOURCE)])) != 4) {
          sr = sprintf(err_msg,
        ...
        SPF(endofstring(echo_string), "\t %.4g %.4g %.4g %.4g",
            pd_ptr->etm[mtrx_index0][ce][(LOG2_MASS)],
            pd_ptr->etm[mtrx_index0][ce][(LOG2_ADVECTION)],
            pd_ptr->etm[mtrx_index0][ce][(LOG2_DIFFUSION)],
            pd_ptr->etm[mtrx_index0][ce][(LOG2_SOURCE)]);
      ...
      case WSTAR:
        ...
        if (fscanf(ifp, "%lf %lf %lf %lf %lf %lf", &(pd_ptr->etm[mtrx_index0][ce][(LOG2_MASS)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_ADVECTION)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_BOUNDARY)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_DIFFUSION)]),
                   &(pd_ptr->etm[mtrx_index0][ce][(LOG2_SOURCE)]),
        ...
        SPF(endofstring(echo_string), "\t %.4g %.4g %.4g %.4g %.4g %.4g",
            pd_ptr->etm[mtrx_index0][ce][(LOG2_MASS)],
            pd_ptr->etm[mtrx_index0][ce][(LOG2_ADVECTION)],
            pd_ptr->etm[mtrx_index0][ce][(LOG2_BOUNDARY)],
            pd_ptr->etm[mtrx_index0][ce][(LOG2_DIFFUSION)],
    ...
    if (pd_ptr->e[mtrx_index0][ce]) {
      ...
      for (n = 1; n < vn_glob[mn]->modes; n++) {
        ...
        pd_ptr->etm[mtrx_index0][cem][(LOG2_ADVECTION)] =
            pd_ptr->etm[mtrx_index0][ce][(LOG2_ADVECTION)];
        pd_ptr->etm[mtrx_index0][cem][(LOG2_BOUNDARY)] =
            pd_ptr->etm[mtrx_index0][ce][(LOG2_BOUNDARY)];
        pd_ptr->etm[mtrx_index0][cem][(LOG2_DIFFUSION)] =
            pd_ptr->etm[mtrx_index0][ce][(LOG2_DIFFUSION)];
     ...
     */
    if ((neqn < 1 || neqn > 76)) {
      GOMA_EH(GOMA_ERROR, "rd_eq_specs: Too many (>76) or too few (<1) eqns. in this mat");
    }
...
int look_for_mat_prop(FILE *imp,              /* ptr to input stream (in)*/
...
int look_for_mat_proptable(FILE *imp,              /* ptr to input stream (in)*/
                           char *search_string,    /* search string (in) */
                           int *MaterialModel,     /* int material model (out)*/
                           dbl *Material_property, /* double value matprop,
                                                    * if constant (out) */
...
int look_for_modal_prop(FILE *imp,                 /* ptr to input stream (in)*/
...
int read_constants(FILE *imp,            /* pointer to file */
...

{
  ...
  if (species_no < 0)
    wspec = 0;
  if (User_constants == NULL) {
    fprintf(stderr, "%s:\t User Defined not allowed yet\n", yo);
    exit(-1);
  }
  if (fgets(line, 255, imp) != NULL) {
    ...
    fprintf(stderr, "%s:\tError @ Constants: USER \n", yo);
    exit(-1);
  }
  return num_const;
}
...
void set_mp_to_unity(const int mn) {
  int i;
  int p;
  int v;
  int w;
...
void usage(const int exit_flag) {
  fprintf(stdout, "Usage: goma [-options] [file]\n\n");
  fprintf(stdout, "Where: [file] is name of overriding input file. If an\n");
  fprintf(stdout, "overriding input file is not specified, then \"input\" must\n");
  ...
  fprintf(stdout, "\t-n INT,     -newton INT         Change # of Newton it'ns to INT.\n");
  fprintf(stdout, "\t-ts FLT,                        Start time of simulation (initial time).\n");
  fprintf(stdout, "\t-te FLT,                        End time of simulation (maximum time).\n");
  fprintf(stdout, "\t-nd,        -nodisplay          Send stdout, stderr to null.\n");
  fprintf(stdout, "\t-ox FILE,   -outexoII FILE      Write FEM to FILE.\n");
  fprintf(stdout, "\t-r FLT,     -relax FLT          Set Newton relaxation to FLT.\n");
  fprintf(stdout, "\t-s FILE,    -soln FILE          Write solution to FILE.\n");
  fprintf(stdout, "\t-se FILE,   -stderr FILE        Redirect stderr to FILE\n");
  fprintf(stdout, "\t-so FILE,   -stdout FILE        Redirect stdout to FILE\n");
  fprintf(stdout, "\t-cb FLT                         Continuation: Start value\n");
  fprintf(stdout, "\t-ce FLT                         Continuation: Final value\n");
  fprintf(stdout, "\t-cd FLT                         Continuation: Path step, ds\n");
  fprintf(stdout, "\t-cn INT                         Continuation: Max number of path steps\n");
  fprintf(stdout, "\t-cmin FLT                       Continuation: Minimum path step\n");
  fprintf(stdout, "\t-cmax FLT                       Continuation: Maximum path step\n");
  fprintf(stdout, "\t-cm INT                         Continuation: Method\n");
  fprintf(stdout, "\t-ct INT                         Continuation: Type\n");
  fprintf(stdout, "\t-c_bc INT                       Continuation: Boundary condition ID\n");
  fprintf(stdout, "\t-c_df INT                       Continuation: BC Data Float ID\n");
  fprintf(stdout, "\t-c_mn INT                       Continuation: Material ID\n");
  fprintf(stdout, "\t-c_mp INT                       Continuation: Method property ID\n");
...
void translate_command_line(int argc, char *argv[], struct Command_line_command **clc, int *nclc)
/*
  ...
  *nclc = -1;
  while (istr < argc) {
    if (*argv[istr] == '-') {
       ...
       */
      if (strcmp(argv[istr], "-nodisplay") == 0 || strcmp(argv[istr], "-nd") == 0) {
        (*nclc)++;
        istr++;
       ...
       */
      else if (strcmp(argv[istr], "-h") == 0 || strcmp(argv[istr], "-help") == 0) {
        (*nclc)++;
        istr++;
       ...
       */
      else if (strcmp(argv[istr], "-v") == 0 || strcmp(argv[istr], "--version") == 0) {
        (*nclc)++;
        istr++;
       ...
       *         programs.
       */
      else if (strcmp(argv[istr], "-se") == 0 || strcmp(argv[istr], "-stderr") == 0) {
        (*nclc)++;
        istr++;
        ...
        if (freopen(clc[*nclc]->string, "w", stderr) == NULL) {
          sr = sprintf(err_msg, "Problem diverting stderr to \"%s\"", clc[*nclc]->string);
          GOMA_EH(GOMA_ERROR, err_msg);
        }
        if (Num_Proc > 1 && Unlimited_Output) {
          fprintf(stderr, "GOMA WARNING: Standard error was redirected to %s on Proc 0 only!\n",
                  clc[*nclc]->string);
          fprintf(stderr, "\t\tThe other processors will still print to the terminal\n");
          fprintf(
              stderr,
       ...
       *         programs.
       */
      else if (strcmp(argv[istr], "-so") == 0 || strcmp(argv[istr], "-stdout") == 0) {
        (*nclc)++;
        istr++;
        ...
        if (freopen(clc[*nclc]->string, "w", stdout) == NULL) {
          sr = sprintf(err_msg, "Problem diverting stdout to \"%s\"", clc[*nclc]->string);
          GOMA_EH(GOMA_ERROR, err_msg);
        }
        if (Num_Proc > 1) {
          fprintf(stderr, "GOMA WARNING: Standard output was redirected to %s on Proc 0 only!\n",
                  clc[*nclc]->string);
          fprintf(stderr, "\t\tThe other processors will still print to the terminal\n");
          fprintf(
              stderr,
       ...
       */
      else if (strcmp(argv[istr], "-input") == 0 || strcmp(argv[istr], "-i") == 0) {
      ...
      /*
       * OPTION -contin: read continuation (guessfile) from alternate file overriding file in
       * 'input'
       */
       ...
       */
      else if (strcmp(argv[istr], "-aprepro") == 0 || strcmp(argv[istr], "-a") == 0) {
        (*nclc)++;
        istr++;
...
/* apply_command_line */
void apply_command_line(struct Command_line_command **clc, int nclc)
/*
 ...
 */
{
  ...
  for (i = 0; i < nclc; i++) {
    if (clc[i]->type == CONTIN_FILE) {
      ...
      fprintf(stdout, "%s%40s= %d ...\n%s%40s= %d ...\n", a, "Continuation", Continuation, b,
              "Continuation", clc[i]->i_val);
      Continuation = clc[i]->i_val;
    } else if (clc[i]->type == CONT_TYPE) {
      fprintf(stdout, "%s%40s= %d ...\n%s%40s= %d ...\n", a, "Continuation Type", cont->upType, b,
              "Continuation Type", clc[i]->i_val);
      ...
      fprintf(stdout, "%s%40s= %d ...\n%s%40s= %d ...\n", a, "Boundary condition data float tag",
              cont->upDFID, b, "Boundary condition data float tag", clc[i]->i_val);
      cont->upDFID = clc[i]->i_val;
    } else if (clc[i]->type == CONT_MTID) {
      fprintf(stdout, "%s%40s= %d ...\n%s%40s= %d ...\n", a, "Material id", cont->upMTID, b,
              "Material id", clc[i]->i_val);
...
struct Data_Table *
setup_table_BC(FILE *ifp, char *input, struct Boundary_Condition *BC_Type, char *echo_string) {
  char err_msg[MAX_CHAR_IN_INPUT];
...
int count_datalines(FILE *ifp, char *input, const char *endlist) {
  /*
...
int look_for_next_string(FILE *ifp, const char *string, char *input, const char ch_term) {
  char err_msg[MAX_CHAR_IN_INPUT];
...
FILE *fopen_aprepro(const char *filename, const char *format) {
...
struct Data_Table *
setup_gd_table_BC(FILE *ifp, char *input, struct Boundary_Condition *BC_Type, char *echo_string) {
  char err_msg[MAX_CHAR_IN_INPUT];
  /*           Function allocates memory for the BC table structure.
  ...
  if (fscanf(ifp, "%80s", input) != 1) {
    sr = sprintf(err_msg, "Cannot find interpolation method for %s on %sID=%d.\n",
                 BC_Type->desc->name1, BC_Type->Set_Type, BC_Type->BC_ID);
    GOMA_EH(GOMA_ERROR, err_msg);
  }
  ...
  if (strcmp(input, "LINEAR") == 0) {
    ...
    sr = sprintf(err_msg, "Unknown interpolation method for %s on %sID=%d.\n", BC_Type->desc->name1,
                 BC_Type->Set_Type, BC_Type->BC_ID);
    GOMA_EH(GOMA_ERROR, err_msg);
  }
...
struct Data_Table *setup_table_MP(FILE *imp, struct Data_Table *table, char *search_string) {
  char err_msg[MAX_CHAR_IN_INPUT];
  ...
  if (fscanf(imp, "%80i", &num_const) != 1) {
    sprintf(err_msg, "Error reading # columns for TABLE model for material property");
    GOMA_EH(GOMA_ERROR, err_msg);
  }

  if ((num_const > 3) && (strcmp(table->f_name, "Saturation") != 0)) {
    sprintf(err_msg, "Error - Multi DOF table lookup limited to bilinear");
    GOMA_EH(GOMA_ERROR, err_msg);
  }
  if ((num_const > 4) && (strcmp(table->f_name, "Saturation") == 0)) {
    sprintf(err_msg, "Error - Multi DOF table lookup limited to bilinear for Saturation");
    GOMA_EH(GOMA_ERROR, err_msg);
  }
  ...
  for (i = 0; i < table->columns - 1; i++) {
    if (fscanf(imp, "%80s", line) != 1) {
      sprintf(err_msg, "Error reading TABLE model for material property for Independent variables");
      GOMA_EH(GOMA_ERROR, err_msg);
    }
  ...
  if (fscanf(imp, "%80s", line) != 1) {
    sprintf(err_msg, "Error reading TABLE %s for material property %s for interpolation",
            table->f_name, line);
    GOMA_EH(GOMA_ERROR, err_msg);
  }
  ...
  if ((strcmp(line, "LINEAR") == 0)) {
    if ((table->columns == 2) ||
        (table->columns == 3 && (strcmp(table->f_name, "Saturation") == 0))) {
...
void rd_table_data(FILE *ifp, char *input, struct Data_Table *table, char *endlist) {
  char err_msg[MAX_CHAR_IN_INPUT];
  ...
  if (table->tablelength == 0)
    GOMA_EH(GOMA_ERROR, "Error reading tabular data . Can't find any points ");

  if (table->tablelength == 1)
    GOMA_EH(GOMA_ERROR, "Error reading tabular data . Need more than 1 point ");

  if ((table->interp_method == QUADRATIC || table->interp_method == BIQUADRATIC ||
       table->interp_method == TRIQUADRATIC) &&
      Num_Pnts % 2 != 1)
    GOMA_EH(GOMA_ERROR, "Need odd number of points for (bi,tri)quadratic interpolation ");
  if (table->interp_method == QUAD_GP && Num_Pnts % 3 != 0)
    GOMA_EH(GOMA_ERROR, "Need 3N points for quadratic gauss point interpolation ");
  ...
  table->f = (double *)smalloc(sizeof(double) * Num_Pnts * (table->columns - table_dim));
    ...
    while (strncmp(endlist, input, len) != 0) /* read until it finds the endlist pattern */
    {
                                                   ...
                                                   */
      {
        ...
        /*  if( table->columns == 2)
          ...
          table->f_name); sprintf(err_msg + strlen(err_msg), "\nRead %d data values prior to
          error.\n", k); GOMA_WH(-1,err_msg);
                  }
                  else
                          k++;
                  snprintf(echo_string, MAX_CHAR_ECHO_INPUT,"\t(%.4g %.4g)", table->t+(k-1),
          table->f+(k-1) );
          }
          ...
          error.\n", k); GOMA_WH(-1,err_msg);
                  ...
                  snprintf(echo_string, MAX_CHAR_ECHO_INPUT,"\t(%.4g %.4g %.4g)", table->t+(k-1),
          table->t2+(k-1), table->f+(k-1) );
          }
          else if(table->columns ==3 &&
                          (table_dim != 2 && strcmp(table->f_name, "Saturation") != 0) )
          {
                  if( sscanf( input, "%lf %lf %lf", table->t+k, table->f+k, table->f+Num_Pnts+k ) !=
          3)
                  {
                          sprintf(err_msg,"\nCannot read three doubles for Table: %s \n",
          table->f_name); sprintf(err_msg + strlen(err_msg), "\nRead %d data values prior to
          error.\n", k); GOMA_WH(-1,err_msg);
                          ...
                          k++;
                  snprintf(echo_string, MAX_CHAR_ECHO_INPUT,"\t(%.4g %.4g %.4g)", table->t+(k-1),
          table->f+(k-1), table->f+Num_Pnts+(k-1) );
          }
          else if(table->columns ==3 && table_dim == 1
                          && strcmp(table->f_name, "Saturation") == 0)
          {
                  if( sscanf( input, "%lf %lf %lf", table->t+k, table->f+Num_Pnts+k, table->f+k ) !=
          3)
                  {
                          sprintf(err_msg,"\nCannot read three doubles for Table: %s \n",
          table->f_name); sprintf(err_msg + strlen(err_msg), "\nRead %d data values prior to
          ...
          table->f+Num_Pnts+(k-1), table->f+(k-1) );
                          ...
                          && strcmp(table->f_name, "Saturation") == 0)
          {
                  if( sscanf( input, "%lf %lf %lf %lf", table->t+k, table->t2+k,
          table->f+Num_Pnts+k, table->f+k  ) != 4)
                  {
          ...
          error.\n", k); GOMA_WH(-1,err_msg);
                          ...
                          k++;
                  snprintf(echo_string, MAX_CHAR_ECHO_INPUT,"\t(%.4g %.4g %.4g %.4g)",
          table->t+(k-1), table->t2+(k-1), table->f+Num_Pnts+(k-1), table->f+(k-1) );
          }
          else if(table->columns ==4 && table_dim == 3 )
          {
                  if( sscanf( input, "%lf %lf %lf %lf", table->t+k, table->t2+k, table->t3+k,
          table->f+k  ) != 4)
                  {
                          sprintf(err_msg,"\nCannot read four doubles for Table: %s \n",
          table->f_name); sprintf(err_msg + strlen(err_msg), "\nRead %d data values prior to
          error.\n", k); GOMA_WH(-1,err_msg);
                          ...
                          k++;
                  snprintf(echo_string, MAX_CHAR_ECHO_INPUT,"\t(%.4g %.4g %.4g %.4g)",
          table->t+(k-1), table->t2+(k-1), table->t3+(k-1), table->f+(k-1) );
          }
          ...
          error.\n", k); GOMA_WH(-1,err_msg);
                          ...
                          k++;
                  snprintf(echo_string, MAX_CHAR_ECHO_INPUT,"\t(%.4g %.4g %.4g %.4g %.4g)",
          table->t+(k-1), table->t2+(k-1), table->f+(k-1), table->f+Num_Pnts+(k-1),
          table->f+2*Num_Pnts+(k-1));
          }
          else
          {
...
table->f+(k-1), table->f+Num_Pnts+(k-1) );
                        ...
                        && strcmp(table->f_name, "Saturation") == 0)
        {
                if( sscanf( input, "%lf %lf %lf %lf", table->t+k, table->t2+k, table->f+Num_Pnts+k,
table->f+k  ) != 4)
                {
    sprintf(err_msg,"\nCannot read four doubles for Table: %s \n", table->f_name);
    sprintf(err_msg + strlen(err_msg), "\nRead %d data values prior to error.\n", k);
    GOMA_WH(-1,err_msg);
                        ...
                        k++;
                snprintf(echo_string, MAX_CHAR_ECHO_INPUT,"\t(%.4g %.4g %.4g %.4g)", table->t+(k-1),
table->t2+(k-1), table->f+Num_Pnts+(k-1), table->f+(k-1) );
        }
        else if(table->columns ==4 && table_dim == 3 )
        {
...
table->f_name); sprintf(err_msg + strlen(err_msg), "\nRead %d data values prior to error.\n", k);
                        ...
                        k++;
                snprintf(echo_string, MAX_CHAR_ECHO_INPUT,"\t(%.4g %.4g %.4g %.4g)", table->t+(k-1),
table->t2+(k-1), table->t3+(k-1), table->f+(k-1) );
        }
        else if(table->columns ==5 && table_dim ==2 )
        {
                if( sscanf( input, "%lf %lf %lf %lf %lf",
...
table->f+2*Num_Pnts+(k-1));
  ...
  snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(Found %d data points in Table : %s)", k,
           table->f_name);
  ECHO(echo_string, echo_file);
        ...
        leave it in. - RBS		*/
  {
    for (i = 1; i < Num_Pnts; i++) {
      ...
      while (j >= 0 && (p < table->t[j])) {
        table->t[j + 1] = table->t[j];
        table->t[j] = p;
        if (table_dim == 2) {
          table->t2[j + 1] = table->t2[j];
          table->t2[j] = p2;
        }
        table->f[j + 1] = table->f[j];
        table->f[j] = p3;
        if ((table->columns - table_dim) > 1) {
  ...
  */
  ...
  if (table_dim > 1 && table->interp_method != BILINEAR) {
    ...
    for (i = 2; i < Num_Pnts; i++) {
      a = SQUARE(table->t[0] - table->t[i - 1]) + SQUARE(table->t2[0] - table->t2[i - 1]);
      b = SQUARE(table->t[0] - table->t[i]) + SQUARE(table->t2[0] - table->t2[i]);
      c = SQUARE(table->t[i] - table->t[i - 1]) + SQUARE(table->t2[i] - table->t2[i - 1]);
      if (table_dim == 3) {
    ...
    if (table_dim == 3) {
      ...
      for (i = iend; i < Num_Pnts; i = i + table->ngrid) {
        j = i - table->ngrid;
        a = SQUARE(table->t[ibegin] - table->t[j]) + SQUARE(table->t2[ibegin] - table->t2[j]) +
            SQUARE(table->t3[ibegin] - table->t3[j]);
        b = SQUARE(table->t[ibegin] - table->t[i]) + SQUARE(table->t2[ibegin] - table->t2[i]) +
            SQUARE(table->t3[ibegin] - table->t3[i]);
        c = SQUARE(table->t[i] - table->t[j]) + SQUARE(table->t2[i] - table->t2[j]) +
      ...
      if (Num_Pnts % table->ngrid2 != 0) {
        ...
        fprintf(stderr, "%d x %d != %d points\n", table->ngrid, table->ngrid2, Num_Pnts);
        GOMA_EH(GOMA_ERROR, "Fatal Table Error");
      }
    }
  }
...
int scan_table_columns(
    int k, char *input, struct Data_Table *table, int table_dim, char *err_msg, char *echo_string) {
  ...
  if (table->columns == 2) {
    ...
    if (sscanf(input, "%lf %lf %lf", table->t + k, table->t2 + k, table->f + k) != 3) {
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%.4g %.4g %.4g)", table->t[k], table->t2[k],
               table->f[k]);
  } else if (table->columns == 3 && (table_dim != 2 && strcmp(table->f_name, "Saturation") != 0)) {
    if (sscanf(input, "%lf %lf %lf", table->t + k, table->f + k, table->f + Num_Pnts + k) != 3) {
      sprintf(err_msg, "\nCannot read three doubles for Table: %s \n", table->f_name);
      sprintf(endofstring(err_msg), "\nRead %d data values prior to error.\n", k);
      err_stat = 1;
    } else
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%.4g %.4g %.4g)", table->t[k], table->f[k],
               table->f[Num_Pnts + k]);
  } else if (table->columns == 3 && table_dim == 1 && strcmp(table->f_name, "Saturation") == 0) {
    if (sscanf(input, "%lf %lf %lf", table->t + k, table->f + Num_Pnts + k, table->f + k) != 3) {
      sprintf(err_msg, "\nCannot read three doubles for Table: %s \n", table->f_name);
      sprintf(endofstring(err_msg), "\nRead %d data values prior to error.\n", k);
      err_stat = 1;
    } else
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%.4g %.4g %.4g)", table->t[k],
               table->f[Num_Pnts + k], table->f[k]);
  } else if (table->columns == 4 && table_dim == 2 && strcmp(table->f_name, "Saturation") == 0) {
    if (sscanf(input, "%lf %lf %lf %lf", table->t + k, table->t2 + k, table->f + Num_Pnts + k,
               table->f + k) != 4) {
      sprintf(err_msg, "\nCannot read four doubles for Table: %s \n", table->f_name);
      sprintf(endofstring(err_msg), "\nRead %d data values prior to error.\n", k);
    ...
    if (sscanf(input, "%lf %lf %lf %lf", table->t + k, table->t2 + k, table->t3 + k,
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%.4g %.4g %.4g %.4g)", table->t[k],
               table->t2[k], table->t3[k], table->f[k]);
  } else if (table->columns == 5 && table_dim == 2) {
    if (sscanf(input, "%lf %lf %lf %lf %lf", table->t + k, table->t2 + k, table->f + k,
               table->f + Num_Pnts + k, table->f + 2 * Num_Pnts + k) != 5) {
      sprintf(err_msg, "\nCannot read five doubles for Table: %s \n", table->f_name);
      sprintf(endofstring(err_msg), "\nRead %d data values prior to error.\n", k);
      err_stat = 1;
    } else
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t(%.4g %.4g %.4g %.4g %.4g)",
               *(table->t + (k - 1)), *(table->t2 + (k - 1)), table->f[k], table->f[Num_Pnts + k],
               table->f[2 * Num_Pnts + k]);
  } else {
    GOMA_EH(GOMA_ERROR, "invalid table column, dimension combination.");
  }
  return (err_stat);
...
void echo_compiler_settings(void) {
  FILE *echo_file;
  ...
  fprintf(echo_file, "%-30s= %s\n", "Linear Stability",
          (Linear_Stability == LSA_NORMAL
               ...
               : (Linear_Stability == LSA_SAVE
                      ...
                      : (Linear_Stability == LSA_3D_OF_2D
                             ? "3D"
                             : (Linear_Stability == LSA_3D_OF_2D_SAVE ? "3Dfile" : "no")))));
...
static double parse_press_datum_input(const char *input)
 ...
 **********************************************************************/
{
  ...
  if (numTok > 2)
    goto L_ERROR;
  if (!interpret_double(tok.tok_ptr[0], &value))
    goto L_ERROR;
  if (numTok == 2) {
   ...
   */
L_ERROR:;
  (void)fprintf(stderr, "%s error: reading pressure datum, %s\n", yo, input);
 ...
 ************************************************************************/
{
   ...
   */
  if (numTok < 2) {
    (void)fprintf(stderr, "%s error: Need at least two arguments for MAT keyword\n%s\n", yo, input);
  ...
  for (i = 0; i < mp_glob[mn]->Num_Matrl_Elem_Blk; i++) {
    if (!interpret_int(tok.tok_ptr[i + 1], mp_glob[mn]->Matrl_Elem_Blk_Ids + i))
      goto L_ERROR;

    SPF_INT_VEC(endofstring(echo_string), mp_glob[mn]->Num_Matrl_Elem_Blk,
   ...
   */
L_ERROR:;
  (void)fprintf(stderr, "%s error: reading MAT line, %s\n", yo, input);

  GOMA_EH(GOMA_ERROR, "MAT line");
}

void read_surface_objects(FILE *ifp, char *input, struct LS_Surf_List *list, int num_surf)

{
  ...
  while (num_surf > 0) {
    ...
    if (strcmp(name, "PLANE") == 0) {
      ...
      if (fscanf(ifp, "%lf %lf %lf %lf", &(s->n[0]), &(s->n[1]), &(s->n[2]), &(s->d)) != 4) {
        GOMA_EH(GOMA_ERROR, "PLANE Level Set initialization objects requires 4 float constants.");
      }

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t\t%s = %s %.4g %.4g %.4g %.4g", "SURF", "PLANE",
               s->n[0], s->n[1], s->n[2], s->d);
      ECHO(echo_string, echo_file);
      ...
      if (fscanf(ifp, "%lf %lf %lf", &(s->center[0]), &(s->center[1]), &(s->r)) != 3) {
        GOMA_EH(GOMA_ERROR, "CIRCLE Level Set initialization objects requires 3 float constants.");
      }
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t\t%s = %s %.4g %.4g %.4g", "SURF", "CIRCLE",
               s->center[0], s->center[1], s->r);
      ECHO(echo_string, echo_file);
      ...
      if (fscanf(ifp, "%lf %lf %lf %lf", &(s->center[0]), &(s->center[1]), &(s->center[2]),
                 &(s->r)) != 4) {
        GOMA_EH(GOMA_ERROR, "SPHERE Level Set initialization objects requires 4 float constants.");
      }

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t\t%s = %s %.4g %.4g %.4g %.4g", "SURF",
               "SPHERE", s->center[0], s->center[1], s->center[2], s->r);
      ECHO(echo_string, echo_file);
      ...
      if ((sfp = fopen(input, "r")) == NULL)
        GOMA_EH(GOMA_ERROR, "Could not open facet list file\n");

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t\t%s = %s %s", "SURF", "FACET_LIST", input);
      ECHO(echo_string, echo_file);

      while (look_forward_optional(sfp, "FACET", input, '=') == 1) {
        if (fscanf(sfp, "%d %d", &num, &num_points) != 2) {
        ...
        for (i = 0; i < num_points; i++) {
          ...
          if (fscanf(sfp, "%d %lf %lf %lf %d", &num, &(r[0]), &(r[1]), &(r[2]), &inflection) != 5)
        ...
        if (num_points == 2) {
          ...
          GOMA_EH(GOMA_ERROR, "Not yet implemented in 3-D\n");
        }
      }
      fclose(sfp);
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t\t%s = %s %d", "SURF", "SS", s->ss_id);
      ECHO(echo_string, echo_file);
      ...
      if (!var_found) {
        GOMA_EH(GOMA_ERROR,
                "Could not identify variable type for Level Set initialization ISOSURFACE");
      }

      if (fscanf(ifp, "%lf", &(s->isoval)) != 1) {
        GOMA_EH(
            GOMA_ERROR,
            "ISOSURFACE Level Set initialization objects requires varname and 1 float constant.");
      }

      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t\t%s = %s %s %.4g", "SURF", "ISOSURFACE", input,
               s->isoval);
      ECHO(echo_string, echo_file);
      ...
      snprintf(echo_string, MAX_CHAR_ECHO_INPUT, "\t\t%s = %s %.4g %.4g %.4g %.4g %.4g %.4g ",
               "SURF", "ARC", s->center[0], s->center[1], s->r, s->n[0], s->n[1], s->d);
      ...
      if (strlen(input) != 0) {
        ...
        if (strcmp(input, "NEGATIVE") == 0) {
          ...
          GOMA_EH(GOMA_ERROR, "Sign designation string for ARC type of LS initialization must be "
      ...
      I = s->Int_Data[0] = read_constants(ifp, &a, 0);
...
struct Data_Table *setup_table_external(char *filename, struct Data_Table *table, char *var_name) {
  char err_msg[MAX_CHAR_IN_INPUT];
  char *yo;
  FILE *datafile = NULL;
...
struct Data_Table *
setup_table_AC(char *filename, struct Data_Table *table, char *var_name, char *interpolation) {
  char err_msg[MAX_CHAR_IN_INPUT];
  char *yo;
  FILE *datafile = NULL;
  yo = "setup_table_AC";
...
FILE *handle_echo_string(char *echo_string, char *echo_file_name) {
  ...
  while ((i < num_open_files) && (strcmp(open_files[i], echo_file_name) != 0))
    i++;
...
int sprintf_int_vec(char *string, int n, int *v) {
  int i = 0;
  char *s1 = string;
...
int sprintf_flt_vec(char *string, int n, float *v) {
  int i = 0;
  char *s1 = string;
...
int sprintf_dbl_vec(char *string, int n, double *v) {
  int i = 0;
  char *s1 = string;
