 ...
 *
 *******************************************************************************/
double viscosity(struct Generalized_Newtonian *gn_local,
  ...
  if ((gn_local->ConstitutiveEquation == NEWTONIAN) ||
    ...
    if (mp->ViscosityModel == USER) {
      ...
      if (fabs(mu) < DBL_SMALL)
        mu = mp->u_viscosity[num + 1];
      ls = ls_save;
    }
     ...
     */
      ...
      if (d_mu != NULL) {
        for (j = 0; j < ei[pg->imtrx]->dof[MASS_FRACTION]; j++) {
          d_mu->C[species][j] = (mu * 2.5 / (1.0 - p_vol_frac)) * bf[MASS_FRACTION]->phi[j];
        }
      }
  ...
  /* this section is for all non-Newtonian models */
    ...
    if (pd->v[pg->imtrx][SHELL_PARTC]) {
      err = suspension_viscosity(gn_local->sus_species_no, gn_local->mu0, gn_local->maxpack,
                                 gn_local->nexp, fv->sh_pc);
      GOMA_EH(err, "suspension_viscosity");
      mu = mp->viscosity;
    } else {
      err = suspension_viscosity(gn_local->sus_species_no, gn_local->mu0, gn_local->maxpack,
                                 gn_local->nexp, fv->c[gn_local->sus_species_no]);
    ...
    err = cure_viscosity(gn_local->cure_species_no, gn_local->mu0, gn_local->gelpoint,
                         gn_local->cureaexp, gn_local->curebexp);
    ...
    if (gn_local->mu0Model == LEVEL_SET) {
      if (d_mu != NULL) {
        err = level_set_property(gn_local->u_mu0[0], gn_local->u_mu0[1], gn_local->u_mu0[2], &mu0,
                                 d_mu->F);
      } else {
        err = level_set_property(gn_local->u_mu0[0], gn_local->u_mu0[1], gn_local->u_mu0[2], &mu0,
                                 NULL);
    ...
    if (gn_local->atexpModel == LEVEL_SET) {
      if (d_mu != NULL) {
        err = level_set_property(gn_local->u_atexp[0], gn_local->u_atexp[1], gn_local->u_atexp[2],
                                 &atexp, d_mu->F);
      } else {
        err = level_set_property(gn_local->u_atexp[0], gn_local->u_atexp[1], gn_local->u_atexp[2],
                                 &atexp, NULL);
    ...
    err =
        bond_viscosity_sh(gn_local->sus_species_no, gn_local->mu0, gn_local->muinf, gn_local->aexp);
    ...
    err = foam_pmdi10_viscosity(gn_local->cure_species_no, gn_local->mu0, gn_local->gelpoint,
                                gn_local->cureaexp, gn_local->curebexp, gn_local->atexp);
    ...
    err = sylgard_viscosity(gn_local->cure_species_no, gn_local->mu0, gn_local->gelpoint,
                            gn_local->cureaexp, gn_local->atexp);
    ...
    err = filled_epoxy_viscosity(gn_local->sus_species_no, gn_local->cure_species_no, gn_local->mu0,
                                 gn_local->maxpack, gn_local->nexp, gn_local->gelpoint,
                                 gn_local->cureaexp, gn_local->curebexp, gn_local->tgel0,
                                 gn_local->atexp);
    ...
    err = foam_epoxy_viscosity(gn_local->sus_species_no, gn_local->cure_species_no, gn_local->mu0,
                               gn_local->gelpoint, gn_local->atexp);
    mu = mp->viscosity;
    if (d_mu != NULL) {
      if (pd->v[pg->imtrx][TEMPERATURE]) {
        for (j = 0; j < ei[pg->imtrx]->dof[TEMPERATURE]; j++) {
...
double carreau_viscosity(struct Generalized_Newtonian *gn_local,
                         dbl gamma_dot[DIM][DIM], /* strain rate tensor */
                         VISCOSITY_DEPENDENCE_STRUCT *d_mu) {
  ...
  if (d_mu != NULL)
    d_mu->gd = (mu0 - muinf) * (nexp - 1.) * lambda * val * val1;
...
double bingham_viscosity(struct Generalized_Newtonian *gn_local,
                         dbl gamma_dot[DIM][DIM], /* strain rate tensor */
                         VISCOSITY_DEPENDENCE_STRUCT *d_mu) {
  ...
  if (d_mu != NULL)
    d_mu->gd = at_shift * (d_yield * visc_cy + (mu0 - muinf + yield) * d_visc_cy * d_shear);
...
double carreau_wlf_viscosity(struct Generalized_Newtonian *gn_local,
                             dbl gamma_dot[DIM][DIM], /* strain rate tensor */
                             VISCOSITY_DEPENDENCE_STRUCT *d_mu) {
  ...
  if (gn_local->atexpModel == LEVEL_SET) {
    if (d_mu != NULL) {
      level_set_property(gn_local->u_atexp[0], gn_local->u_atexp[1], gn_local->u_atexp[2], &atexp,
                         d_mu->F);
    } else {
      level_set_property(gn_local->u_atexp[0], gn_local->u_atexp[1], gn_local->u_atexp[2], &atexp,
                         NULL);
  ...
  if (gn_local->wlfc2Model == LEVEL_SET) {
    if (d_mu != NULL) {
      level_set_property(gn_local->u_wlfc2[0], gn_local->u_wlfc2[1], gn_local->u_wlfc2[2], &wlfc2,
                         d_mu->F);
    } else {
      level_set_property(gn_local->u_wlfc2[0], gn_local->u_wlfc2[1], gn_local->u_wlfc2[2], &wlfc2,
                         NULL);
  ...
  if (d_mu != NULL)
    d_mu->gd = at_shift * ((mu0 - muinf) * d_visc_cy * d_shear);
 ...
 *                      Cmaxpack  = binding solid fraction, i.e. solid fraction for infinite
 *viscosity nexp   = exponent
 *
                         ...
                         dbl C)       /* Concentration  */
{
  ...
  if (nexp > 0.0 || (C > 0.0 && C < 0.98 * maxpack)) {
    ...
    if (pd->v[pg->imtrx][SHELL_PARTC]) {
      ...
      /* d2mu/dc2 .... */
      mp->d2_viscosity[SHELL_PARTC] =
          mu0 * nexp * (nexp - 1.0) * pow(1.0 - C / maxpack, nexp - 2.) / maxpack / maxpack;
    }
...
double carreau_suspension_viscosity(struct Generalized_Newtonian *gn_local,
                                    dbl gamma_dot[DIM][DIM], /* strain rate tensor  */
                                    VISCOSITY_DEPENDENCE_STRUCT *d_mu) {
  int a, b;
  int var, var_offset;
  ...
  if (d_mu != NULL) {
    ...
    mp->d2_viscosity[SHEAR_RATE] =
        d_mu->gd * ((aexp - 1) / (lambda * gammadot) + lambda * (nexp - 1. - aexp) / (1 + val2));
  }
...
double powerlaw_suspension_viscosity(struct Generalized_Newtonian *gn_local,
                                     dbl gamma_dot[DIM][DIM], /* strain rate tensor */
                                     VISCOSITY_DEPENDENCE_STRUCT *d_mu) {
  int a, b;
  int var, var_offset;
  ...
  if (d_mu != NULL)
    d_mu->gd = mu * (nexp - 1.0) / (gammadot + offset);

  mp->d2_viscosity[SHEAR_RATE] = mu * (nexp - 1.0) * (nexp - 2.0) / SQUARE(gammadot + offset);
...
int epoxy_viscosity(int species, /* species number for cure equation */
                    ...
                    dbl Aexp)    /* exponent for thermal viscosity dependence */
{
  ...
  if (pd->v[pg->imtrx][var]) {
    if (T <= 0.) {
      ...
      mp->d2_viscosity[var] = -mu * SQUARE(Aexp / (T * T)) + 2. * mu * Aexp / (T * T * T);
    }
  }
  ...
  if (pd->v[pg->imtrx][var]) {
    if (alpha < alpha_g) {
      ...
      mp->d2_viscosity[MAX_VARIABLE_TYPES + species] = mu * deriv * deriv + mu * d_deriv;
    } else {
...
int foam_pmdi10_viscosity(int species, /* species number for cure equation */
                          dbl mu0,     /* monomer reference temperature viscosity */
                          dbl alpha_g, /* extent of reaction at the gel point */
                          dbl A,       /* exponent for constitutive equation */
                          dbl B,       /* exponent for constitutive equation */
                          dbl norm_E)  /* Normalized activation energy */
{
  ...
  if (T <= 0) {
    ...
    mp->d_viscosity[var] = mu * norm_E / (T * T) + muL * mp->d_volumeFractionGas[var] * partial;
  }
  ...
  if (pd->gv[var]) {
    if (alpha < alpha_g && alpha > 1e-8) {
      ...
      for (w = 0; w < pd->Num_Species; w++) {
        mp->d_viscosity[MAX_VARIABLE_TYPES + species] =
            mu * deriv + muL * mp->d_volumeFractionGas[MAX_VARIABLE_TYPES + w] * partial;
      }
...
int sylgard_viscosity(int species, /* species number for cure equation */
                      dbl mu0,     /* monomer reference temperature viscosity */
                      dbl alpha_g, /* extent of reaction at the gel point */
                      dbl A,       /* exponent for constitutive equation */
                      dbl Aexp)    /* exponent for thermal viscosity dependence */
 ...
 *                      mu0       = solvent viscosity
 *                      Cmaxpack  = binding solid fraction, i.e. solid fraction for infinite
 *viscosity nexp      = exponent alpha     = extent of reaction alpha_g   = extent of reaction at
 *the gel point A, B      = exponent for cure behavior Aexp      = exponent for temperature
...
int filled_epoxy_viscosity(int species_sus, /* species num, solid volume fraction*/
                           ...
                           dbl Atexp)       /* exponent for thermal viscosity function */
{
  ...
  if (nexp > 0.0 || (vf > 0.0 && vf <= (maxpack - .01))) {
    ...
    mp->viscosity = mu;
    mp->d_viscosity[MAX_VARIABLE_TYPES + species_sus] = -mu * nexp / (maxpack - vf);
  ...
  if (Num_Var_In_Type[pg->imtrx][var]) {
    if (alpha < alpha_g) {
      deriv = 8. / 3. * alpha / (alpha_g2 - alpha2) - duu_dT * (A * T_g / (1. - A * alpha));
      ...
      mp->d2_viscosity[MAX_VARIABLE_TYPES + species_cur] = mu * deriv * deriv + mu * d_deriv;
    } else {
 ...
 *
 ******************************************************************************************/
int foam_epoxy_viscosity(int species_fluor, int species_cur, dbl mu0, dbl alpha_g, dbl Aexp) {
  dbl mu; /* viscosity */
  dbl T;  /* temperature */
  dbl ratio;
  dbl volFac = 1.0;
  ...
  if (Num_Var_In_Type[pg->imtrx][MASS_FRACTION]) {
    if (alpha < alpha_g) {
      ...
      mp->d_viscosity[MAX_VARIABLE_TYPES + species_cur] = mu * deriv;
      mp->d_FlowingLiquid_viscosity[MAX_VARIABLE_TYPES + species_cur] = muL * deriv;
    } else {
      mp->d_viscosity[MAX_VARIABLE_TYPES + species_cur] = 0.0;
      mp->d_FlowingLiquid_viscosity[MAX_VARIABLE_TYPES + species_cur] = 0.0;
    }

    if (mp->volumeFractionGas >= 0.0 && mp->volumeFractionGas <= 0.99) {
 ...
 *
 *****************************************************************************/
int thermal_viscosity(dbl mu0,  /* reference temperature fluid viscosity */
...
int cure_viscosity(int species, /* species num, solid volume fraction        */
                   dbl mu0,     /* carrier fluid viscosity                   */
                   dbl alpha_g, /* extent of reaction at the gel point       */
                   dbl A,       /* exponent for constitutive equation        */
                   dbl B)       /* exponent for constitutive equation        */
...
double bond_viscosity(struct Generalized_Newtonian *gn_local,
                      dbl gamma_dot[DIM][DIM], /* strain rate tensor */
                      VISCOSITY_DEPENDENCE_STRUCT *d_mu) {
  /* Local Variables */
  dbl mu;       /* viscosity */
  ...
  if (nn <= 0.0) {
    ...
    if (DOUBLE_NONZERO(gammadot) && DOUBLE_NONZERO(at_shift)) {
      ...
      d_yield =
          tau_y * ((1. + at_shift * fexp * gammadot) * exp_term) / at_shift / SQUARE(gammadot);
      d_yield_d_at = d_yield * gammadot / at_shift;
    } else {
      yield = tau_y * fexp;
      d_yield = 0.;
...
double carreau_wlf_conc_viscosity(struct Generalized_Newtonian *gn_local,
                                  dbl gamma_dot[DIM][DIM], /* strain rate tensor */
                                  VISCOSITY_DEPENDENCE_STRUCT *d_mu,
                                  const int const_eqn) {
  ...
  if (d_mu != NULL && pd->v[pg->imtrx][var]) {
    if (const_eqn == CARREAU_WLF_CONC_PL) {
      if (nonvol_conc > 0.) {
        dmudC =
            mu + at_conc * at_conc * at_shift * at_shift * (mu0 - muinf) * d_visc_cy * d_shear_d_ac;
...
int ls_modulate_viscosity(double *mu1,
                          double mu2,
                          double width,
                          double pm_minus,
                          double pm_plus,
...
void copy_pF_to_F(int phase) {
  int a;

  fv->F = fv->pF[phase];

  for (a = 0; a < pd->Num_Dim; a++)
    fv->grad_F[a] = fv->grad_pF[phase][a];
}
 ...
 *
 *******************************************************************************/
double flowing_liquid_viscosity(VISCOSITY_DEPENDENCE_STRUCT *d_flow_vis) {
  ...
  if (mp->FlowingLiquidViscosityModel == CONSTANT) {
    ...
    (void)molten_glass_viscosity(&flow_vis, d_flow_vis->T, mp->u_FlowingLiquid_viscosity);
  }
  ...
  if (ls != NULL && mp->mp2nd != NULL && (mp->mp2nd->FlowingLiquidViscosityModel == CONSTANT)) {
