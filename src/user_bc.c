 ...
 ******************************************************************************/
{
  ...
  */
  ...
  /* Comment this out FIRST!!!!! */
  // GOMA_EH(GOMA_ERROR,"No SHELL_P_OPEN_USER model implemented");
  ...
  if (!first_time) {
    if (Porous_liq_inventory <= u_bc[1]) {
      ...
      *func = fv->sh_p_open - u_bc[0];
      ...
      first_time = 1;
      time_off = time;
      GOMA_EH(GOMA_ERROR, "exit here we have reached the finite load");
    }
  } else {
...

{
  ...
  /* Comment this out FIRST!!!!! */
  /* GOMA_EH(GOMA_ERROR,"No user YFLUX_USER  model implemented"); */
...
int user_gibbs_criterion(const double fsnormal[MAX_PDIM], /* Vector of free surface
                                                           * normal components */
...

{
  ...
  if (imodel == CIRCLE) {
    ...
    pos = fabs(r_circ) - sqrt(SQUARE(fv->x[0] - circ_center_x) + SQUARE(fv->x[1] - circ_center_y) +
                              SQUARE(fv->x[2] - circ_center_z));

    /* 2D only for now */

    if (pd->Num_Dim < 3)
      GOMA_EH(GOMA_ERROR, "USE CA_OR_FIX instead of CA_EDGE_OR_FIX");
    ...
    if ((sign_of(pos) == sign_of(sign_orig)) && (fabs(pos) > 1.e-6)) {
      *ipin = 0;
      return (1);
    ...
    GOMA_EH(GOMA_ERROR, "No user CA_EDGE_OR_FIX model implemented");
  } else {
    GOMA_EH(GOMA_ERROR, "Don't Recognize CA_EDGE_OR_FIX model");
  }
  return (1);
...
void force_user_surf(double func[DIM],
  ...
  */
  ...
  /* Comment this out FIRST!!!!! */
  GOMA_EH(GOMA_ERROR, "No FORCE_USER model implemented. Check-routine. You may be commented out!");
  ...
  */
  ...
  /*  if(time < p[0])
          ...
          GOMA_EH(GOMA_ERROR," ran out of bounds in time baby");
...
******************************************************************************/
{
  ...
  *func = PHI + (i / i0) * cratio * (2.0 / (aa + ac)) * RTF;
  ...
  for (j_id = 0; j_id < ei[pg->imtrx]->dof[var]; j_id++) {
    ...
    d_func[0][MAX_VARIABLE_TYPES][j_id] = -(i / i0) * (cratio / cH2) * RTF / (aa + ac) * phi_j;
  }
